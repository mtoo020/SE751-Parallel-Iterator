...
public interface Sheet {
	/**
	 * Returns the cell specified at this row and at this column. If a
	 * column/row combination forms part of a merged group of cells then (unless
	 * it is the first cell of the group) a blank cell will be returned
	 *
	 * @param column
	 *            the column number
	 * @param row
	 ...
	 * found, then null is returned. The search is performed on a row by row
	 * basis, so the lower the row number, the more efficiently the algorithm
	 * will perform. This method differs from the findCell method in that only
	 ...
	 * @param row
	 *            the row number
	 * @return the row format, which may be the default format if no format is
	 *         specified
	 */
