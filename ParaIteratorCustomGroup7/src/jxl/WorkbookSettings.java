 ...
 * workbook properties. Use of this bean is not mandatory, and its absence will
 * merely result in workbooks being read/written using the default settings
 */
public final class WorkbookSettings {
	 ...
	 * worksheet. For processeses reading many small workbooks inside a WAS it
	 * might be necessary to reduce the default size
	 */
	private int initialFileSize;

	/**
	 * The amount of memory allocated to the array containing the workbook data
	 * when its current amount is exhausted.
	 */
	private int arrayGrowSize;

	/**
	 * Flag to indicate whether the drawing feature is enabled or not Drawings
	 * deactivated using -Djxl.nodrawings=true on the JVM command line Activated
	 * by default or by using -Djxl.nodrawings=false on the JVM command line
	 */
	private boolean drawingsDisabled;

	/**
	 * Flag to indicate whether the name feature is enabled or not Names
	 * deactivated using -Djxl.nonames=true on the JVM command line Activated by
	 * default or by using -Djxl.nonames=false on the JVM command line
	 */
	 ...
	 * not. As a rule of thumb, it is desirable to enable garbage collection
	 * when reading large spreadsheets from a batch process or from the command
	 * line, but better to deactivate the feature when reading large
	 * spreadsheets within a WAS, as the calls to System.gc() not only garbage
	 * collect the junk in JExcelApi, but also in the webservers JVM and can
	 * cause significant slowdown GC deactivated using -Djxl.nogc=true on the
	 * JVM command line Activated by default or by using -Djxl.nogc=false on the
	 * JVM command line
	 */
	private boolean gcDisabled;

	/**
	 * Flag to indicate whether the rationalization of cell formats is disabled
	 * or not. Rationalization is enabled by default, but may be disabled for
	 * performance reasons. It can be deactivated using -Djxl.norat=true on the
	 * JVM command line
	 */
	...
	/**
	 * Flag to indicate whether the copying of additional property sets are
	 * disabled
	 */
	...
	/**
	 * Flag to indicate whether a temporary file should be used when writing out
	 * the workbook
	 */
	private boolean useTemporaryFileDuringWrite;

	/**
	 * The directory for used for the temporary file during write. If this is
	 * NULL, the default system directory is used
	 */
	private File temporaryFileDuringWriteDirectory;

	/**
	 * The locale. Normally this is the same as the system locale, but there may
	 * be cases (eg. where you are uploading many spreadsheets from foreign
	 * sources) where you may want to specify the locale on an individual
	 * worksheet basis The locale may also be specified on the command line
	 * using the lang and country System properties eg. -Djxl.lang=en
	 * -Djxl.country=UK for UK English
	 */
	 ...
	 * different from the default platform encoding if processing spreadsheets
	 ...
	 *
	 * The EXCEL9FILE record indicates the file was written by Excel 2000. It
	 * has no record data field and is C0010000h. Any application other than
	 * Excel 2000 that edits the file should not write out this record.
	 *
	...
	/**
	 * Write access user name. When not set (null) then we set it to Java Excel
	 * API + Version number
	 */
	private String writeAccess;

	/**
	 * The HIDEOBJ record stores options selected in the Options dialog,View
	 * tab.
	 */
	private int hideobj;
	...
	// 5 megabytes
	private static final int DEFAULT_ARRAY_GROW_SIZE = 1024 * 1024; // 1
																	// megabyte
	...
	public WorkbookSettings() {
		...
		try {
			...
			mergedCellCheckingDisabled = Boolean
					.getBoolean("jxl.nomergedcellchecks");
			formulaReferenceAdjustDisabled = Boolean
					.getBoolean("jxl.noformulaadjust");
			...
			useTemporaryFileDuringWrite = Boolean
					.getBoolean("jxl.usetemporaryfileduringwrite");
			String tempdir = System
					.getProperty("jxl.temporaryfileduringwritedirectory");
	...
	/**
	 * Sets the amount of memory by which to increase the amount of memory
	 * allocated to storing the workbook data. For processeses reading many
	 * small workbooks inside a WAS it might be necessary to reduce the default
	 * size Default value is 1 megabyte
	 *
	 * @param sz
	 ...
	 * inside a WAS it might be necessary to reduce the default size Default
	...
	/**
	 * Sets whether or not to rationalize the cell formats before writing out
	 * the sheet. The default value is true
	 *
	 * @param r
	...
	/**
	 * Sets whether or not to enable any property sets (such as macros) to be
	 * copied along with the workbook Leaving this feature enabled will result
	 * in the JXL process using more memory
	 *
	 * @param r
	...
	/**
	 * Accessor to set the suppress warnings flag. Due to the change in logging
	 * in version 2.4, this will now set the warning behaviour across the JVM
	 ...
	 *
	 * @return TRUE if formulas are adjusted following row/column
	 *         inserts/deletes FALSE otherwise
	 */
	...
	/**
	 * Sets the locale used by JExcelApi to generate the spreadsheet. Setting
	 * this value has no effect on the language or region of the generated excel
	 * file
	 *
	 * @param l
	 *            the locale
	...
	/**
	 * Gets the function names. This is used by the formula parsing package in
	 * order to get the locale specific function names for this particular
	 * workbook
	 *
	 ...
	 * has no effect when writing out the spreadsheet
	 *
	...
	/**
	 * Sets the character set. This is only used when the spreadsheet is read,
	 * and has no effect when the spreadsheet is written
	 *
	 * @param cs
	...
	/**
	 * Returns the two character ISO 3166 mnemonic used by excel for its
	 * regional settings
	 * 
	...
	/**
	 * Accessor for the temporary file during write. If this is set, then when
	 * the workbook is written a temporary file will be used to store the
	 * interim binary data, otherwise it will take place in memory. Setting this
	 * flag involves an assessment of the trade-offs between memory usage and
	 * performance
	 *
	 * @return TRUE if a temporary is file is used during writing, FALSE
	 *         otherwise
	 */
	...
	/**
	 * Sets whether a temporary file is used during the generation of the
	 * workbook. If not set, the workbook will take place entirely in memory.
	...
	/**
	 * Used in conjunction with the UseTemporaryFileDuringWrite setting to set
	 * the target directory for the temporary files. If this is not set, the
	 * system default temporary directory is used. This has no effect unless the
	 * useTemporaryFileDuringWrite setting is TRUE
	 *
	 * @param dir
	...
	/**
	 * Used in conjunction with the UseTemporaryFileDuringWrite setting to set
	 * the target directory for the temporary files. This value can be NULL, in
	 * which case the normal system default temporary directory is used instead
	 *
	 * @return the temporary directory used during write, or NULL if it is not
	 *         set
	 */
	 ...
	 *
	 * Possible values are: HIDEOBJ_HIDE_ALL, HIDEOBJ_SHOW_ALL and
