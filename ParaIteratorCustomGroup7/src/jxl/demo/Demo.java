...
/**
 * The main demo class which interprets the command line switches in order to
 * determine how to call the demo programs The demo program uses stdout as its
...
public class Demo {
	...
	private static void displayHelp() {
		System.err
				.println("Command format:  Demo [-unicode] [-csv] [-hide] excelfile");
		System.err.println("                 Demo -xml [-format]  excelfile");
		System.err
				.println("                 Demo -readwrite|-rw excelfile output");
		System.err
				.println("                 Demo -biffdump | -bd | -wa | -write | -formulas | -features | -escher | -escherdg excelfile");
		System.err
				.println("                 Demo -ps excelfile [property] [output]");
		System.err
				.println("                 Demo -version | -logtest | -h | -help");

	}

	/**
	 * The main method. Gets the worksheet and then uses the API within a simple
	 * loop to print out the spreadsheet contents as comma separated values
	 * 
	 * @param args
	...
	public static void main(String args[]) {
		...
		if (write == false && readwrite == false && formulas == false
				&& biffdump == false && jxlversion == false
				&& propertysets == false && features == false
				&& escher == false && escherdg == false) {
			for (int i = 0; i < args.length - 1; i++) {
				if (args[i].equals("-unicode")) {
					...
					System.err
							.println("Command format:  CSV [-unicode] [-xml|-csv] excelfile");
					System.exit(1);
				}
			}
		}
		...
		try {
			if (write) {
				...
				Workbook w = Workbook.getWorkbook(new File(file));
				EscherDrawingGroup f = new EscherDrawingGroup(w, System.out,
						encoding);
				w.close();
			} else if (biffdump) {
	...
	private static void findTest(Workbook w) {
		...
		if (range != null) {
			for (int i = 0; i < range.length; i++) {
				...
				logger.info("nonadjacent top left contents:  "
						+ c.getContents());

				c = range[i].getBottomRight();
				logger.info("nonadjacent bottom right contents:  "
						+ c.getContents());
			}
		}
