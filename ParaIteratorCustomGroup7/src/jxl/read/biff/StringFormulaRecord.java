 ...
 */
class StringFormulaRecord extends CellValue implements LabelCell, FormulaData,
		StringFormulaCell {
	/**
	 * The logger
	 */
	...
	/**
	 * Constructs this object from the raw data. We need to use the excelFile to
	 * retrieve the String record which follows this formula record
	 *
	 * @param t
	 *            the raw data
	 * @param excelFile
	 ...
	 */
	public StringFormulaRecord(Record t, File excelFile, FormattingRecords fr,
		...
		while (nextRecord.getType() == Type.CONTINUE) {
			...
			System.arraycopy(nextRecord.getData(), 1, d, stringData.length,
					nextRecord.getLength() - 1);
			stringData = d;
			nextRecord = excelFile.peek();
		}
	 ...
	 *            the workbook settings
	 */
	public StringFormulaRecord(Record t, FormattingRecords fr,
			ExternalSheet es, WorkbookMethods nt, SheetImpl si) {
		super(t, fr, si);
	...
	public String getFormula() throws FormulaException {
		if (formulaString == null) {
			...
			FormulaParser fp = new FormulaParser(tokens, this, externalSheet,
					nameTable, getSheet().getWorkbook().getSettings());
			fp.parse();
