...
public class NameRecord extends RecordData {
	...
	NameRecord(Record t, WorkbookSettings ws, int ind) {
		...
		try {
			...
			if (data[pos] == cellReference) {
				...
				int columnMask = IntegerHelper.getInt(data[pos + 5],
						data[pos + 6]);
				...
				while (pos < data.length) {
					...
					columnMask = IntegerHelper.getInt(data[pos + 7],
							data[pos + 8]);
					c1 = columnMask & 0xff;
					...
					columnMask = IntegerHelper.getInt(data[pos + 9],
							data[pos + 10]);
					c2 = columnMask & 0xff;
				...
				if (pos < data.length && data[pos] != cellReference
	...
	NameRecord(Record t, WorkbookSettings ws, int ind, Biff7 dummy) {
		...
		try {
			...
			if (data[pos] == cellReference) {
				int sheet = IntegerHelper
						.getInt(data[pos + 11], data[pos + 12]);
				...
				while (pos < data.length) {
					sheet1 = IntegerHelper.getInt(data[pos + 11],
							data[pos + 12]);
