...
class SharedFormulaRecord {
	 ...
	 *
	 * @param fr
	 *            the formula record to test for membership of this group
	...
	Cell[] getFormulas(FormattingRecords fr, boolean nf) {
		...
		if (templateFormula.getType() == CellType.NUMBER_FORMULA) {
			...
			if (fr.isDate(templateFormula.getXFIndex())) {
				templateFormula = new SharedDateFormulaRecord(snfr, fr, nf,
						sheet, snfr.getFilePos());
	...
	/**
	 * Accessor for the template formula. Called when a shared formula has, for
	 * some reason, specified an inappropriate range and it is necessary to
	 * retrieve the template from a previously available shared formula
	 */
