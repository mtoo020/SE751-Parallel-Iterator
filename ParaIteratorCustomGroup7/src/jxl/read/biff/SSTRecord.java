...
class SSTRecord extends RecordData {
	...
	public SSTRecord(Record t, Record[] continuations, WorkbookSettings ws) {
		...
		int pos = 0;
		System.arraycopy(getRecord().getData(), 0, data, 0, getRecord()
				.getLength());
		...
		uniqueStrings = IntegerHelper
				.getInt(data[4], data[5], data[6], data[7]);
	...
	private void readStrings(byte[] data, int offset, WorkbookSettings ws) {
		...
		for (int i = 0; i < uniqueStrings; i++) {
			...
			if (extendedString) {
				// Read in cchExtRst
				extendedRunLength = IntegerHelper.getInt(data[pos],
	 ...
	 *            the original source
	 * @param bah
	 *            holder for the new byte array
	 * @param pos
	 *            the current position in the source
	 * @param ascii
	 *            holder for a return ascii flag
	 * @param numChars
	 *            the number of chars in the string
	 * @return the number of bytes read from the source
	 */
	private int getChars(byte[] source, ByteArrayHolder bah, int pos,
			BooleanHolder ascii, int numChars) {
		int i = 0;
		...
		bytesRead += getContinuedString(source, bah, bytesRead, i, ascii,
				numChars - charsRead);
		return bytesRead;
	}
	 ...
	 *            the original bytes
	 * @param bah
	 *            the holder for the new bytes
	 * @param destPos
	 *            the des pos
	 * @param contBreakIndex
	 *            the index of the continuation break
	 * @param ascii
	 *            the ascii flag holder
	 * @param charsLeft
	 *            the number of chars left in the array
	 * @return the number of bytes read in the continued string
	 */
	private int getContinuedString(byte[] source, ByteArrayHolder bah,
			int destPos, int contBreakIndex, BooleanHolder ascii, int charsLeft) {
		...
		while (charsLeft > 0) {
			...
			if (ascii.value && source[breakpos] == 0) {
				...
				int length = contBreakIndex == continuationBreaks.length - 1 ? charsLeft
						: Math.min(charsLeft,
								continuationBreaks[contBreakIndex + 1]
										- breakpos - 1);

				System.arraycopy(source, breakpos + 1, bah.bytes, destPos,
						length);
				destPos += length;
				bytesRead += length + 1;
				charsLeft -= length;
				...
				int length = contBreakIndex == continuationBreaks.length - 1 ? charsLeft * 2
						: Math.min(charsLeft * 2,
								continuationBreaks[contBreakIndex + 1]
										- breakpos - 1);

				// It looks like the string continues as Unicode too. That's
				// handy
				System.arraycopy(source, breakpos + 1, bah.bytes, destPos,
						length);
				...
				// continuation it is in Unicode. This impacts the allocated
				// array
