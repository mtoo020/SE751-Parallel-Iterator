...
/**
 * Reads the sheet. This functionality was originally part of the SheetImpl
 * class, but was separated out in order to simplify the former class
 */
final class SheetReader {
	...
	/**
	 * An array of column info records. They are held this way before they are
	 * transferred to the more convenient array
	 */
	 ...
	 *            the excel file
	 * @param sb
	 *            the bof record which indicates the start of the sheet
	 * @param wb
	 *            the bof record which indicates the start of the sheet
	 * @param wp
	 *            the workbook which this sheet belongs to
	 * @param sp
	 *            the start position of the sheet bof in the excel file
	 * @param sh
	 *            the sheet
	 * @param nf
	 *            1904 date record flag
	 * @exception BiffException
	 */
	SheetReader(File f, SSTRecord sst, FormattingRecords fr, BOFRecord sb,
			BOFRecord wb, boolean nf, WorkbookParser wp, int sp, SheetImpl sh) {
		excelFile = f;
		sharedStrings = sst;
	...
	private void addCell(Cell cell) {
		...
		if (cell.getRow() < numRows && cell.getColumn() < numCols) {
			...
			/*
			 * logger.warn("Cell " + CellReferenceHelper.getCellReference
	...
	final void read() {
		...
		while (cont) {
			...
			if (type == Type.DIMENSION) {
				...
				LabelSSTRecord label = new LabelSSTRecord(r, sharedStrings,
						formattingRecords, sheet);
				addCell(label);
				...
				if (formattingRecords.isDate(rkr.getXFIndex())) {
					DateCell dc = new DateRecord(rkr, rkr.getXFIndex(),
							formattingRecords, nineteenFour, sheet);
					addCell(dc);
				} else {
					addCell(rkr);
				}
			} else if (type == Type.HLINK) {
				HyperlinkRecord hr = new HyperlinkRecord(r, sheet,
						workbookSettings);
				hyperlinks.add(hr);
				...
				if (mergedCells == null) {
					...
					Range[] newMergedCells = new Range[mergedCells.length
							+ mc.getRanges().length];
					System.arraycopy(mergedCells, 0, newMergedCells, 0,
							mergedCells.length);
					System.arraycopy(mc.getRanges(), 0, newMergedCells,
							mergedCells.length, mc.getRanges().length);
				...
				for (int i = 0; i < num; i++) {
					...
					NumberValue nv = new NumberValue(mulrk.getRow(),
							...
							RKHelper.getDouble(mulrk.getRKNumber(i)), ixf,
							formattingRecords, sheet);

					if (formattingRecords.isDate(ixf)) {
						DateCell dc = new DateRecord(nv, ixf,
								formattingRecords, nineteenFour, sheet);
						addCell(dc);
					} else {
						nv.setNumberFormat(formattingRecords
								.getNumberFormat(ixf));
						addCell(nv);
					}
				}
				...
				if (formattingRecords.isDate(nr.getXFIndex())) {
					DateCell dc = new DateRecord(nr, nr.getXFIndex(),
							formattingRecords, nineteenFour, sheet);
					addCell(dc);
				} else {
					addCell(nr);
				...
				BooleanRecord br = new BooleanRecord(r, formattingRecords,
						sheet);
				...
				settings.setPrintGridLines(printGridLinesRecord
						.getPrintGridLines());
				...
				settings.setDisplayZeroValues(window2Record
						.getDisplayZeroValues());
				settings.setSelected(true);
				settings.setPageBreakPreviewMode(window2Record
						.isPageBreakPreview());
				...
				if (!workbookSettings.getDrawingsDisabled()) {
					...
					Comment comment = (Comment) comments.remove(new Integer(nr
							.getObjectId()));
					...
					if (comment == null) {
						...
						addCellComment(comment.getColumn(), comment.getRow(),
								comment.getText(), comment.getWidth(),
								comment.getHeight());
					}
				}
				...
				if (sharedFormula == null) {
					...
					SharedFormulaRecord lastSharedFormula = (SharedFormulaRecord) sharedFormulas
							.get(sharedFormulas.size() - 1);
				...
				SharedFormulaRecord sfr = new SharedFormulaRecord(r,
						sharedFormula, workbook, workbook, sheet);
				sharedFormulas.add(sfr);
				sharedFormula = null;
			} else if (type == Type.FORMULA || type == Type.FORMULA2) {
				FormulaRecord fr = new FormulaRecord(r, excelFile,
						formattingRecords, workbook, workbook, sheet,
						workbookSettings);
				...
				if (fr.isShared()) {
					...
					try {
						...
						if (fr.getFormula().getType() == CellType.NUMBER_FORMULA) {
							NumberFormulaRecord nfr = (NumberFormulaRecord) fr
									.getFormula();
							if (formattingRecords.isDate(nfr.getXFIndex())) {
								cell = new DateFormulaRecord(nfr,
										formattingRecords, workbook, workbook,
										nineteenFour, sheet);
							}
						}
						...
						// Something has gone wrong trying to read the formula
						// data eg. it
						// might be unsupported biff7 data
						logger.warn(CellReferenceHelper.getCellReference(
								cell.getColumn(), cell.getRow())
								+ " " + e.getMessage());
					}
				}
				...
				if (workbookBof.isBiff8()) {
					lr = new LabelRecord(r, formattingRecords, sheet,
							workbookSettings);
				} else {
					lr = new LabelRecord(r, formattingRecords, sheet,
							workbookSettings, LabelRecord.biff7);
				}
				addCell(lr);
				...
				lr = new RStringRecord(r, formattingRecords, sheet,
						workbookSettings, RStringRecord.biff7);
				addCell(lr);
				...
				if (!rr.isDefaultHeight() || !rr.matchesDefaultFontHeight()
						|| rr.isCollapsed() || rr.hasDefaultFormat()
				...
				if (sr.getInitialized()) {
					...
					settings.setPaperSize(PaperSize.getPaperSize(sr
							.getPaperSize()));
					...
					settings.setVerticalPrintResolution(sr
				...
				ConditionalFormatRangeRecord cfrr = new ConditionalFormatRangeRecord(
						r);
				...
				if (workbookBof.isBiff8()) {
					...
					dr = new VerticalPageBreaksRecord(r,
							VerticalPageBreaksRecord.biff7);
				}
				...
				if (!workbookSettings.getCellValidationDisabled()) {
					...
					if (dvlr.getObjectId() == -1) {
						if (msoRecord != null && objRecord == null) {
							// there is a drop down associated with this data
							// validation
							if (drawingData == null) {
					...
					} else if (objectIds.contains(new Integer(dvlr
							.getObjectId()))) {
						dataValidation = new DataValidation(dvlr);
					} else {
						logger.warn("object id "
								+ dvlr.getObjectId()
								+ " referenced "
				...
				if (!workbookSettings.getCellValidationDisabled()) {
					DataValiditySettingsRecord dvsr = new DataValiditySettingsRecord(
							r, workbook, workbook, workbook.getSettings());
					if (dataValidation != null) {
						dataValidation.add(dvsr);
						addCellValidation(dvsr.getFirstColumn(),
								dvsr.getFirstRow(), dvsr.getLastColumn(),
								dvsr.getLastRow(), dvsr);
					} else {
				...
				if (!workbookSettings.getDrawingsDisabled()) {
					// sometimes excel writes out continue records instead of
					// drawing
					// records, so forcibly hack the stashed continue record
					// into
					// a drawing record
					if (msoRecord == null && continueRecord != null) {
						logger.warn("Cannot find drawing record - using continue record");
						msoRecord = new MsoDrawingRecord(
								continueRecord.getRecord());
						continueRecord = null;
					}
				...
				if (!workbookSettings.getDrawingsDisabled()) {
					if (msoRecord != null) {
						// For form controls, a rogue MSODRAWING record can crop
						// up
				...
				settings.setRecalculateFormulasBeforeSave(cmr
						.getRecalculateOnSave());
			} else if (type == Type.GUTS) {
				GuttersRecord gr = new GuttersRecord(r);
				maxRowOutlineLevel = gr.getRowOutlineLevel() > 0 ? gr
						.getRowOutlineLevel() - 1 : 0;
				maxColumnOutlineLevel = gr.getColumnOutlineLevel() > 0 ? gr
						.getRowOutlineLevel() - 1 : 0;
				...
				if (br.isChart()) {
					if (!workbook.getWorkbookBof().isBiff8()) {
						...
						if (!workbookSettings.getDrawingsDisabled()) {
							Chart chart = new Chart(msoRecord, objRecord,
									drawingData, startpos, excelFile.getPos(),
				...
				// represents the end of the sheet as well as the end of the
				// chart
				if (sheetBof.isChart()) {
					cont = false;
				}
	...
	/**
	 * Reverts the shared formula passed in to an ordinary formula and adds it
	 * to the list
	 *
	 * @param f
	 *            the formula
	...
	private Cell revertSharedFormula(BaseSharedFormulaRecord f) {
		...
		FormulaRecord fr = new FormulaRecord(f.getRecord(), excelFile,
				formattingRecords, workbook, workbook,
				FormulaRecord.ignoreSharedFormula, sheet, workbookSettings);
		...
		try {
			...
			if (fr.getFormula().getType() == CellType.NUMBER_FORMULA) {
				...
				if (formattingRecords.isDate(fr.getXFIndex())) {
					cell = new DateFormulaRecord(nfr, formattingRecords,
							workbook, workbook, nineteenFour, sheet);
				}
			}
			...
			// might be unsupported biff7 data
			logger.warn(CellReferenceHelper.getCellReference(fr.getColumn(),
					fr.getRow())
					+ " " + e.getMessage());
	 ...
	 *
	 * @param col
	 *            the column for the comment
	 * @param row
	 *            the row for the comment
	 * @param text
	 *            the comment text
	 * @param width
	 ...
	 */
	private void addCellComment(int col, int row, String text, double width,
			double height) {
		...
		if (c == null) {
			...
			MulBlankCell mbc = new MulBlankCell(row, col, 0, formattingRecords,
					sheet);
	 ...
	 */
	private void addCellValidation(int col1, int row1, int col2, int row2,
		...
		for (int row = row1; row <= row2; row++) {
			for (int col = col1; col <= col2; col++) {
				...
				if (c == null) {
					MulBlankCell mbc = new MulBlankCell(row, col, 0,
							formattingRecords, sheet);
					CellFeatures cf = new CellFeatures();
					cf.setValidationSettings(dvsr);
					mbc.setCellFeatures(cf);
					addCell(mbc);
				} else if (c instanceof CellFeaturesAccessor) { // Check to see
																// if the cell
																// already
																// contains a
																// comment
	 ...
	 *            the obj record
	 * @param msoRecord
	 *            the mso drawing record read in earlier
	 * @param comments
	 *            the hash map of comments
	 */
	private void handleObjectRecord(ObjRecord objRecord,
			MsoDrawingRecord msoRecord, HashMap comments) {
		if (msoRecord == null) {
		...
		try {
			...
			if (objRecord.getType() == ObjRecord.PICTURE) {
				...
				Drawing drawing = new Drawing(msoRecord, objRecord,
						drawingData, workbook.getDrawingGroup(), sheet);
			...
			if (objRecord.getType() == ObjRecord.EXCELNOTE) {
				...
				Comment comment = new Comment(msoRecord, objRecord,
						drawingData, workbook.getDrawingGroup(),
						workbookSettings);
			...
			if (objRecord.getType() == ObjRecord.COMBOBOX) {
				...
				ComboBox comboBox = new ComboBox(msoRecord, objRecord,
						drawingData, workbook.getDrawingGroup(),
						workbookSettings);
			...
			if (objRecord.getType() == ObjRecord.CHECKBOX) {
				...
				CheckBox checkBox = new CheckBox(msoRecord, objRecord,
						drawingData, workbook.getDrawingGroup(),
						workbookSettings);
			...
			if (objRecord.getType() == ObjRecord.BUTTON) {
				...
				Button button = new Button(msoRecord, objRecord, drawingData,
						workbook.getDrawingGroup(), workbookSettings);
			...
			if (objRecord.getType() == ObjRecord.TEXT) {
				logger.warn(objRecord.getType() + " Object on sheet \""
						+ sheet.getName() + "\" not supported - omitting");
				...
				if (workbook.getDrawingGroup() != null) // can be null for Excel
														// 95
				{
	...
	/**
	 * Handle any cells which fall outside of the bounds specified within the
	 * dimension record
	 */
