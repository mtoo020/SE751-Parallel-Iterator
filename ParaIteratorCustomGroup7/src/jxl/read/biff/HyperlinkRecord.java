...
/**
 * A number record. This is stored as 8 bytes, as opposed to the 4 byte RK
 * record
 */
public class HyperlinkRecord extends RecordData implements Hyperlink {
	...
	HyperlinkRecord(Record t, Sheet s, WorkbookSettings ws) {
		...
		int options = IntegerHelper.getInt(data[28], data[29], data[30],
				data[31]);
		...
		int descbytes = 0;
		if (description) {
			int descchars = IntegerHelper.getInt(data[startpos],
					data[startpos + 1], data[startpos + 2], data[startpos + 3]);
		...
		int targetbytes = 0;
		if (targetFrame) {
			int targetchars = IntegerHelper.getInt(data[startpos],
					data[startpos + 1], data[startpos + 2], data[startpos + 3]);
		...
		if (linkType == urlLink) {
			...
			try {
				...
				int bytes = IntegerHelper.getInt(data[startpos],
						data[startpos + 1], data[startpos + 2],
						data[startpos + 3]);
				...
				CellReferenceHelper
						.getCellReference(firstColumn, firstRow, sb1);
			...
			int chars = IntegerHelper.getInt(data[32], data[33], data[34],
					data[35]);
