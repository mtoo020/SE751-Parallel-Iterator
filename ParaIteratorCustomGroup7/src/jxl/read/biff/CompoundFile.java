...
/**
 * Reads in and defrags an OLE compound compound file (Made public only for the
 * PropertySets demo)
 */
public final class CompoundFile extends BaseCompoundFile {
	...
	/**
	 * The original OLE stream, organized into blocks, which can appear at any
	 ...
	 *
	 * @param d
	 *            the raw data of the ole stream
	 * @param ws
	 *            the workbook settings
	...
	public CompoundFile(byte[] d, WorkbookSettings ws) throws BiffException {
		...
		sbdStartBlock = IntegerHelper.getInt(data[SMALL_BLOCK_DEPOT_BLOCK_POS],
				data[SMALL_BLOCK_DEPOT_BLOCK_POS + 1],
				data[SMALL_BLOCK_DEPOT_BLOCK_POS + 2],
				data[SMALL_BLOCK_DEPOT_BLOCK_POS + 3]);
		rootStartBlock = IntegerHelper.getInt(data[ROOT_START_BLOCK_POS],
				data[ROOT_START_BLOCK_POS + 1], data[ROOT_START_BLOCK_POS + 2],
				data[ROOT_START_BLOCK_POS + 3]);
		extensionBlock = IntegerHelper.getInt(data[EXTENSION_BLOCK_POS],
				data[EXTENSION_BLOCK_POS + 1], data[EXTENSION_BLOCK_POS + 2],
		...
		for (int i = 0; i < bbdBlocks; i++) {
			bigBlockDepotBlocks[i] = IntegerHelper.getInt(d[pos], d[pos + 1],
					d[pos + 2], d[pos + 3]);
			pos += 4;
		}
		...
		for (int j = 0; j < numExtensionBlocks; j++) {
			...
			for (int i = bbdBlocks; i < bbdBlocks + blocksToRead; i++) {
				bigBlockDepotBlocks[i] = IntegerHelper.getInt(d[pos],
			...
			if (bbdBlocks < numBigBlockDepotBlocks) {
				extensionBlock = IntegerHelper.getInt(d[pos], d[pos + 1],
						d[pos + 2], d[pos + 3]);
			}
		}
	...
	private void readBigBlockDepot() {
		...
		for (int i = 0; i < numBigBlockDepotBlocks; i++) {
			...
			for (int j = 0; j < BIG_BLOCK_SIZE / 4; j++) {
				bigBlockChain[index] = IntegerHelper.getInt(data[pos],
	...
	private void readSmallBlockDepot() throws BiffException {
		...
		for (; blockCount <= bigBlockChain.length && sbdBlock != -2; blockCount++) {
			...
			smallBlockChain = new int[smallBlockChain.length + BIG_BLOCK_SIZE
					/ 4];
			...
			for (int j = 0; j < BIG_BLOCK_SIZE / 4; j++) {
				smallBlockChain[index] = IntegerHelper.getInt(data[pos],
						data[pos + 1], data[pos + 2], data[pos + 3]);
				pos += 4;
				index++;
			}
		...
		if (blockCount > bigBlockChain.length) {
			// Attempted to read more blocks than the block chain contains
			// entries
	...
	/**
	 * Recursively searches the property storages in hierarchy order for the
	 * appropriate name. This is the public version which is invoked from the
	 * writable version when copying a sheet with addition property sets.
	 */
	 ...
	 * 
	 * @param index
	 *            the index of the property storage
	 * @return the property storage record
	 */
	...
	private byte[] getBigBlockStream(PropertyStorage ps) {
		...
		while (block != -2 && count < numBlocks) {
			...
			System.arraycopy(data, pos, streamData, count * BIG_BLOCK_SIZE,
					BIG_BLOCK_SIZE);
			count++;
	...
	private byte[] getSmallBlockStream(PropertyStorage ps) throws BiffException {
		...
		for (; blockCount <= smallBlockChain.length && block != -2; blockCount++) {
			...
			System.arraycopy(rootdata, pos, sbdata, olddata.length,
					SMALL_BLOCK_SIZE);
			block = smallBlockChain[block];

			if (block == -1) {
				logger.warn("Incorrect terminator for small block stream "
						+ ps.name);
	...
	/**
	 * Reads the block chain from the specified block and returns the data as a
