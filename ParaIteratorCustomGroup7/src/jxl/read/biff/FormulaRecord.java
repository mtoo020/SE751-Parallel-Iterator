...
class FormulaRecord extends CellValue {
	...
	/**
	 * Static class for a dummy override, indicating that the formula passed in
	 * is not a shared formula
	 */
	 ...
	 * NumberFormulaRecord or a StringFormulaRecord depending on whether this
	 * formula represents a numerical calculation or not
	 *
	 * @param t
	 *            the raw data
	 * @param excelFile
	 ...
	 * @param es
	 *            the workbook, which contains the external sheet references
	 * @param nt
	 *            the name table
	 * @param si
	 *            the sheet
	 ...
	 */
	public FormulaRecord(Record t, File excelFile, FormattingRecords fr,
		...
		if ((grbit & 0x08) != 0) {
			...
			if (data[6] == 0 && data[12] == -1 && data[13] == -1) {
				...
				formula = new SharedStringFormulaRecord(t, excelFile, fr, es,
						nt, si, ws);
			} else if (data[6] == 3 && data[12] == -1 && data[13] == -1) {
				// We have a string which evaluates to null
				formula = new SharedStringFormulaRecord(t, excelFile, fr, es,
						nt, si, SharedStringFormulaRecord.EMPTY_STRING);
				...
				formula = new SharedBooleanFormulaRecord(t, excelFile, value,
						fr, es, nt, si);
			} else {
	 ...
	 *            the name table
	 * @param i
	 *            a dummy override to indicate that we don't want to do any
	 *            shared formula processing
	 * @param si
	 *            the sheet impl
	 * @param ws
	 *            the workbook settings
	 */
	public FormulaRecord(Record t, File excelFile, FormattingRecords fr,
			ExternalSheet es, WorkbookMethods nt, IgnoreSharedFormula i,
