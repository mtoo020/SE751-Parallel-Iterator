 ...
 * the spreadsheet
 */
public class WorkbookParser extends Workbook implements ExternalSheet,
		WorkbookMethods {
	/**
	 * The logger
	 */
	...
	/**
	 * The country record (containing the language and regional settings) for
	 * this workbook
	 */
	...
	/**
	 * Gets the sheets within this workbook. NOTE: Use of this method for very
	 * large worksheets can cause performance and out of memory problems. Use
	 * the alternative method getSheet() to retrieve each sheet individually
	 *
	...
	/**
	 * Package protected function which gets the real internal sheet index based
	 * upon the external sheet reference. This is used for extern sheet
	 * references which are specified in formulas
	 *
	 * @param index
	 *            the external sheet reference
	...
	public String getExternalSheetName(int index) {
		...
		if (sr.getType() == SupbookRecord.INTERNAL) {
			// It's an internal reference - get the name from the boundsheets
			// list
			if (firstTab == 65535) {
				firstTabName = "#REF";
			} else {
				BoundsheetRecord br = (BoundsheetRecord) boundsheets
						.get(firstTab);
	...
	protected void parse() throws BiffException, PasswordException {
		...
		while (bof != null && getNumberOfSheets() < boundsheets.size()) {
			...
			if (bof.isWorksheet()) {
				// Read the sheet in
				SheetImpl s = new SheetImpl(excelFile, sharedStrings,
						formattingRecords, bof, workbookBof, nineteenFour, this);

				BoundsheetRecord br = (BoundsheetRecord) boundsheets
						.get(getNumberOfSheets());
	...
	/**
	 * Accessor for the formattingRecords, used by the WritableWorkbook when
	...
	/**
	 * Accessor for the externSheet, used by the WritableWorkbook when creating
	 * a copy of this
	 *
	...
	/**
	 * Accessor for the MsoDrawingGroup, used by the WritableWorkbook when
	 * creating a copy of this
	 *
	...
	/**
	 * Accessor for the supbook records, used by the WritableWorkbook when
	 * creating a copy of this
	 *
	...
	/**
	 * Accessor for the name records. Used by the WritableWorkbook when creating
	 * a copy of this
	 *
	...
	/**
	 * Accessor for the fonts, used by the WritableWorkbook when creating a copy
	 * of this
	 * 
	...
	/**
	 * Method used when parsing formulas to make sure we are trying to parse a
	...
	/**
	 * Accessor for the CompoundFile. For this feature to return non-null value,
	 * the propertySets feature in WorkbookSettings must be enabled and the
	 * workbook must contain additional property sets. This method is used
	 * during the workbook copy
	 *
	 * @return the base compound file if it contains additional data items and
	 *         property sets are enabled
	 */
