 ...
 * cells, or lines of cells (grouped by Row or Column) In order to simplify this
 * class due to code bloat, the actual reading logic has been delegated to the
 * SheetReaderClass. This class' main responsibility is now to implement the API
 * methods declared in the Sheet interface
 */
public class SheetImpl implements Sheet {
	...
	SheetImpl(File f, SSTRecord sst, FormattingRecords fr, BOFRecord sb,
			BOFRecord wb, boolean nf, WorkbookParser wp) throws BiffException {
		excelFile = f;
		sharedStrings = sst;
		...
		if (sheetBof.isChart()) {
			// Set the start pos to include the bof so the sheet reader can
			// handle it
	...
	/**
	 * Gets all the cells on the specified row. The returned array will be
	 * stripped of all trailing empty cells
	 *
	 * @param row
	...
	/**
	 * Gets all the cells on the specified column. The returned array will be
	 * stripped of all trailing empty cells
	 *
	 * @param col
	...
	/**
	 * Gets the column info record for the specified column. If no column is
	...
	public ColumnInfoRecord getColumnInfo(int col) {
		if (!columnInfosInitialized) {
			...
			while (i.hasNext()) {
				...
				int endcol = Math.min(columnInfos.length - 1,
						cir.getEndColumn());
	...
	/**
	 * Clears out the array of cells. This is done for memory allocation reasons
	 * when reading very large sheets
	 */
	final void clear() {
		cells = null;
		mergedCells = null;
	...
	final void readSheet() {
		...
		SheetReader reader = new SheetReader(excelFile, sharedStrings,
				formattingRecords, sheetBof, workbookBof, nineteenFour,
				workbook, startPosition, this);
		reader.read();
		...
		if (columnInfosArray.size() > 0) {
			ColumnInfoRecord cir = (ColumnInfoRecord) columnInfosArray
					.get(columnInfosArray.size() - 1);
				...
				if (nr.getBuiltInName() == BuiltInName.PRINT_AREA) {
					if (nr.getRanges().length > 0) {
						...
						settings.setPrintArea(rng.getFirstColumn(),
								rng.getFirstRow(), rng.getLastColumn(),
								rng.getLastRow());
					}
					...
					for (int i = 0; i < nr.getRanges().length; i++) {
						...
						if (rng.getFirstColumn() == 0
								&& rng.getLastColumn() == 255) {
	...
	/**
	 * Gets the row record. Usually called by the cell in the specified row in
	 ...
	 * @param row
	 *            the row number
	 * @return the row height, or the default height if the row has no specified
	 *         format
	...
	/**
	 * Accessor for the environment specific print record, invoked when copying
	 * sheets
	 *
	...
	public ConditionalFormat[] getConditionalFormats() {
		ConditionalFormat[] formats = new ConditionalFormat[conditionalFormats
				.size()];
