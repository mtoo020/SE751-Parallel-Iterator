 ...
 */
public abstract class BaseSharedFormulaRecord extends CellValue implements
		FormulaData {
	/**
	...
	/**
	 * The position of the next record in the file. Used when looking for for
	 * subsequent records eg. a string value
	 */
	private int filePos;
	 ...
	 *            the sheet
	 * @param pos
	 *            the position of the next record in the file
	 */
	public BaseSharedFormulaRecord(Record t, FormattingRecords fr,
			ExternalSheet es, WorkbookMethods nt, SheetImpl si, int pos) {
		super(t, fr, si);
		externalSheet = es;
		nameTable = nt;
		filePos = pos;
	...
	/**
	 * In case the shared formula is not added for any reason, we need to expose
	 * the raw record data , in order to try again
	 *
