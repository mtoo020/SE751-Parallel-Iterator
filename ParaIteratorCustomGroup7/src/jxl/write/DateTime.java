 ...
 * their UTC value. If this is not desired (eg. if the date entered represents
 * an interval eg. 9.83s for the 100m world record, then use the overloaded
 * constructor which indicate that the date passed in was created under the GMT
 * timezone. It is important that when the date was created, an instruction like
 * Calendar.setTimeZone(TimeZone.getTimeZone("GMT")) was made prior to that
 */
public class DateTime extends DateRecord implements WritableCell, DateCell {
	...
	/**
	 * Constructor which takes the format for the cell and an indicator as to
	 * whether this cell is a full date time or purely just a time eg. if the
	 * spreadsheet is to contain the world record for 100m, then the value would
	 * be 9.83s, which would be indicated as just a time
	 *
	 * @param c
	 *            the column
	 * @param r
	 ...
	 *            the style
	 * @param tim
	 *            flag indicating that this represents a time
	 * @param d
	 *            the date
	 */
	...
	/**
	 * A constructor called by the worksheet when creating a writable version of
	 * a spreadsheet that has been read in
	 *
	 * @param dc
	 *            the date to copy
	 */
	...
	/**
	 * Sets the date for this cell, performing the necessary timezone
	 * adjustments
	 *
	 * @param d
	 *            the date
