...
/**
 * Allows an image to be created, or an existing image to be manipulated Note
 * that co-ordinates and dimensions are given in cells, so that if for example
 * the width or height of a cell which the image spans is altered, the image
 * will have a correspondign distortion
 */
public class WritableImage extends Drawing {
	...
	/**
	 * Image anchor properties which will move and resize an image along with
	 * the cells
	 */
	public static ImageAnchorProperties MOVE_AND_SIZE_WITH_CELLS = Drawing.MOVE_AND_SIZE_WITH_CELLS;

	/**
	 * Image anchor properties which will move an image when cells are inserted
	 * or deleted
	 */
	public static ImageAnchorProperties MOVE_WITH_CELLS = Drawing.MOVE_WITH_CELLS;

	/**
	 * Image anchor properties which will leave an image unaffected when other
	 * cells are inserted, removed or resized
	 */
	 ...
	 *
	 * @param x
	 *            the column number at which to position the image
	 * @param y
	 *            the row number at which to position the image
	 * @param width
	 *            the number of columns cells which the image spans
	 * @param height
	 *            the number of rows which the image spans
	 * @param image
	 *            the source image file
	 */
	public WritableImage(double x, double y, double width, double height,
			File image) {
	...
	/**
	 * Accessor for the image dimensions Note that the actual size of the
	 * rendered image will depend on the width of the columns it spans
	 *
	 * @param c
	...
	/**
	 * Accessor for the image dimensions Note that the actual size of the
	 * rendered image will depend on the height of the rows it spans
	 *
	 * @param c
