 ...
 * applications in order to present numerical values to the appropriate level of
 * accuracy. The string format used to create a number format adheres to the
 * standard java specification, and JExcelAPI makes the necessary modifications
 * so that it is rendered in Excel as the nearest possible equivalent. Once
 * created, this may be used within a CellFormat object, which in turn is a
 * parameter passed to the constructor of the Number cell
 */
public class NumberFormat extends NumberFormatRecord implements DisplayFormat {
	...
	/**
	 * Constant format string for the Euro currency symbol where it precedes the
	 * format
	 */
	...
	/**
	 * Constructor, taking in the non-Java compliant number format. This may be
	 * used for currencies and more complex custom formats, which will not be
	 * subject to the standard validation rules. As there is no validation,
	 * there is a resultant risk that the generated Excel file will be corrupt
	 *
	 * USE THIS CONSTRUCTOR ONLY IF YOU ARE CERTAIN THAT THE NUMBER FORMAT YOU
	 * ARE USING IS EXCEL COMPLIANT
	 *
	 * @param format
	 *            the format string
	 * @param dummy
