...
/**
 * A user specified cell format, which may be reused across many cells. The
 * constructors takes parameters, such as font details and the numerical date
 * formats, which specify to Excel how cells with this format should be
 * displayed. Once a CellFormat has been added to a Cell which has been added to
 * a sheet, then the CellFormat becomes immutable (to prevent unforeseen effects
 * on other cells which share the same format). Attempts to call the various
 * set... functions on a WritableCellFormat after this time will result in a
 * runtime exception.
 */
public class WritableCellFormat extends CellXFRecord {
	/**
	 * A default constructor, which uses the default font and format. This
	 * constructor should be used in conjunction with the more advanced
	 * two-phase methods setAlignment, setBorder etc.
	 */
	...
	/**
	 * A constructor which specifies a date/number format for Cells which use
	 * this format object
	 *
	 * @param format
	 *            the format
	...
	/**
	 * Sets whether or not this XF record locks the cell. For this to have any
	 * effect, the sheet containing cells with this format must also be locke3d
	 *
	 * @param l
	 *            the locked flag
	 * @exception WriteException
