...
class SSTRecord extends WritableRecordData {
	 ...
	 * characters not added, due to space constraints. In the event of this
	 * being non-zero, a continue record will be needed
	 *
	 * @param s
	 *            the string to add
	...
	public int add(String s) {
		...
		if (byteCount >= maxBytes - 5) {
			return s.length() > 0 ? s.length() : -1; // need to return some
														// non-zero
