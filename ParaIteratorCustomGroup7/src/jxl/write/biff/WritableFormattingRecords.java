...
/**
 * Handles the Format and XF record indexing. The writable subclass instantiates
 * the predetermined list of XF records and formats present in every Excel
 * Workbook
 */
public class WritableFormattingRecords extends FormattingRecords {
	...
	/**
	 * Constructor. Instantiates the prerequisite list of formats and styles
	 * required by all Excel workbooks
	 * 
	 * @param f
	 *            the list of Fonts
	 * @param styles
	 *            the list of style clones
	 */
	public WritableFormattingRecords(Fonts f, Styles styles) {
		...
		try {
			...
			StyleXFRecord sxf = new StyleXFRecord(styles.getArial10Pt(),
			...
			sxf = new StyleXFRecord(getFonts().getFont(2),
					NumberFormats.DEFAULT);
			sxf.setLocked(true);
			sxf.setCellOptions(0xf400);
			addStyle(sxf);

			sxf = new StyleXFRecord(getFonts().getFont(3),
					NumberFormats.DEFAULT);
			sxf.setLocked(true);
			sxf.setCellOptions(0xf400);
			addStyle(sxf);
			 ...
			 * ("\"$\"#,##0_);\\(\"$\"#,##0\\)",5); addFormat(fr);
			 * 
			 * fr = new FormatRecord ("\"$\"#,##0_);[Red]\\(\"$\"#,##0\\)", 6);
			 * addFormat(fr);
			 * 
			 * fr = new FormatRecord ("\"$\"#,##0.00_);\\(\"$\"#,##0.00\\)", 7);
			 * addFormat(fr);
			 * 
			 * fr = new FormatRecord
			 * ("\"$\"#,##0.00_);[Red]\\(\"$\"#,##0.00\\)", 8); addFormat(fr);
			 * 
			 * fr = new FormatRecord
			 * ("_(\"$\"* #,##0_);_(\"$\"* \\(#,##0\\);_(\"$\"* \"-\"_);_(@_)",
			 * 0x2a); // outputFile.write(fr);
			 * 
			 * fr = new FormatRecord
			 * ("_(* #,##0_);_(* \\(#,##0\\);_(* \"-\"_);_(@_)", 0x2e); //
			 * outputFile.write(fr);
			 * 
			 * fr = new FormatRecord (
			 * "_(\"$\"* #,##0.00_);_(\"$\"* \\(#,##0.00\\);_(\"$\"* \"-\"??_);_(@_)"
			 * , 0x2c); // outputFile.write(fr);
			 * 
			 * fr = new FormatRecord
