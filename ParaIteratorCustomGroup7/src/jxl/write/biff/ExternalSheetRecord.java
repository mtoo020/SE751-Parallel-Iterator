...
/**
 * An external sheet record, used to maintain integrity when formulas are copied
 * from read databases
 */
class ExternalSheetRecord extends WritableRecordData {
	...
	public ExternalSheetRecord(jxl.read.biff.ExternalSheetRecord esf) {
		...
		for (int i = 0; i < esf.getNumRecords(); i++) {
			xti = new XTI(esf.getSupbookIndex(i), esf.getFirstTabIndex(i),
	...
	/**
	 * Gets the extern sheet index for the specified parameters, creating a new
	 * xti record if necessary
	 * 
	 * @param supbookind
	 *            the internal supbook reference
	 * @param sheetind
	 *            the sheet index
	 */
