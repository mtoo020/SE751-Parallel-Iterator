...
/**
 * A formula record. Parses the string passed in to deduce the set of formula
 * records
 */
public class FormulaRecord extends CellValue implements FormulaData {
	...
	protected FormulaRecord(int c, int r, FormulaRecord fr) {
		...
		System.arraycopy(fr.formulaBytes, 0, formulaBytes, 0,
				formulaBytes.length);
	}
	...
	/**
	 * Initializes the string and the formula bytes. In order to get access to
	 * the workbook settings, the object is not initialized until it is added to
	 * the sheet
	 *
	 * @param ws
	...
	private void initialize(WorkbookSettings ws, ExternalSheet es,
		...
		try {
			...
			logger.warn(e.getMessage()
					...
					+ getSheet().getName()
					+ "!"
					+ CellReferenceHelper.getCellReference(getColumn(),
							getRow()));
	 ...
	 *            the name table
	 */
	private void initializeCopiedFormula(WorkbookSettings ws, ExternalSheet es,
			WorkbookMethods nt) {
		try {
	...
	/**
	 * Called when the cell is added to the worksheet. Overrides the method in
	 * the base class in order to get a handle to the WorkbookSettings so that
	 * this formula may be initialized
	 * 
	 * @param fr
	...
	/**
	 * A dummy implementation to keep the compiler quiet. This object needs to
	 * be instantiated from ReadFormulaRecord
	 *
	 * @param col
