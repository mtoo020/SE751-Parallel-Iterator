...
/**
 * Contains all the merged cells, and the necessary logic for checking for
 * intersections and for handling very large amounts of merging
 */
class MergedCells {
	/**
	 * The logger
	...
	/**
	 * Adds the range to the list of merged cells. Does no checking at this
	 * stage
	 *
	 * @param range
	 *            the range to add
		...
		for (Iterator mci = ranges.iterator(); mci.hasNext();) {
			...
			while (i.hasNext() && !intersects) {
				...
				if (range.intersects(r)) {
					logger.warn("Could not merge cells "
							+ r
	...
	/**
	 * Checks the cell ranges for intersections, or if the merged cells contains
