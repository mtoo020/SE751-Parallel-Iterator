...
/**
 * A writable sheet. This class contains implementation of all the writable
 * sheet methods which may be invoke by the API
 */
class WritableSheetImpl implements WritableSheet {
	...
	/**
	 * The data validations on this page. Used to store data validations from a
	 * read sheet
	 */
	...
	/**
	 * The writable cells on this sheet which may have validation added to them
	 */
	...
	/**
	 * Drawings modified flag. Set to true if the drawings list has been
	 * modified
	 */
	 ...
	 */
	private final static char[] illegalSheetNameCharacters = new char[] { '*',
			':', '?', '\\' };
	 ...
	 * 
	 * @param fr
	 *            the formatting records used by the workbook
	 * @param of
	 *            the output file to write the binary data
	 * @param f
	 *            the fonts used by the workbook
	 * @param n
	 *            the name of this sheet
	 * @param ss
	 *            the shared strings used by the workbook
	 * @param ws
	 *            the workbook settings
	 */
	public WritableSheetImpl(String n, File of, FormattingRecords fr,
			SharedStrings ss, WorkbookSettings ws, WritableWorkbookImpl ww) {
		name = validateName(n);
		outputFile = of;
	 ...
	 * getCell(CellReferenceHelper.getColumn(loc),
	 * CellReferenceHelper.getRow(loc)) and its implicit performance overhead
	 * for string parsing. As such,this method should therefore be used
	 * sparingly
	 *
	 * @param loc
	 *            the cell reference
	 ...
	 */
	public Cell findCell(String contents, int firstCol, int firstRow,
			int lastCol, int lastRow, boolean reverse) {
		CellFinder cellFinder = new CellFinder(this);
		return cellFinder.findCell(contents, firstCol, firstRow, lastCol,
				lastRow, reverse);
	}
	 ...
	 *            the last row within the range
	 * @param lastCol
	 *            the last column within the ranage
	 * @param reverse
	 *            indicates whether to perform a reverse search or not
	 * @return the Cell whose contents match the parameter, null if not found
	 */
	public Cell findCell(Pattern pattern, int firstCol, int firstRow,
			int lastCol, int lastRow, boolean reverse) {
		CellFinder cellFinder = new CellFinder(this);
		return cellFinder.findCell(pattern, firstCol, firstRow, lastCol,
				lastRow, reverse);
	}
	...
	/**
	 * Inserts a blank row into this spreadsheet. If the row is out of range of
	 * the rows in the sheet, then no action is taken
	 *
	 * @param row
	 *            the row to insert
	 */
	 ...
	 * range of the columns in the sheet, then no action is taken. If the max
	 * column on the sheet has been reached, then the last column entry gets
	 * dropped
	 *
	 * @param col
	...
	/**
	 * Removes a column from this spreadsheet. If the column is out of range of
	 * the columns in the sheet, then no action is taken
	 *
	 * @param col
	...
	/**
	 * Removes a row from this spreadsheet. If the row is out of range of the
	 * columns in the sheet, then no action is taken
	 *
	 * @param row
	 *            the row to remove
	 */
	public void removeRow(int row) {
		...
		// Adjust any drawings
		/*
		 * if (drawings != null) { for (Iterator drawingIt = drawings.iterator()
		 * ; drawingIt.hasNext() ; ) { DrawingGroupObject dgo =
		 * (DrawingGroupObject) drawingIt.next(); dgo.removeRow(row); } }
		 */
	...
	/**
	 * Adds the cell to this sheet. If the cell has already been added to this
	 * sheet or another sheet, a WriteException is thrown. If the position to be
	 * occupied by this cell is already taken, the incumbent cell is replaced.
	 * The cell is then marked as referenced, and its formatting information
	 * registered with the list of formatting records updated if necessary The
	 * RowsExceededException may be caught if client code wishes to explicitly
	 * trap the case where too many rows have been written to the current sheet.
	 * If this behaviour is not desired, it is sufficient simply to handle the
	 * WriteException, since this is a base class of RowsExceededException
	 * 
	 ...
	 *            the cell to add
	 */
	public void addCell(WritableCell cell) throws WriteException,
			RowsExceededException {
		if (cell.getType() == CellType.EMPTY) {
			if (cell != null && cell.getCellFormat() == null) {
				// return if it's a blank cell with no particular cell
				// formatting
				// information
				return;
			}
		...
		boolean curSharedValidation = (curcell != null
				...
				&& curcell.getCellFeatures().getDVParser() != null && curcell
				.getCellFeatures().getDVParser().extendedCellsValidation());
		...
		if (cell.getCellFeatures() != null
			...
			logger.warn("Cannot add cell at "
					...
					+ CellReferenceHelper.getCellReference(
							dvp.getFirstColumn(), dvp.getFirstRow())
					+ "-"
	...
	/**
	 * Gets the row record at the specified row number, growing the array as
	 * needs dictate
	 * 
	 * @param row
	 *            the row number we are interested in
	...
	RowRecord getRowRecord(int row) throws RowsExceededException {
		...
		if (row >= rows.length) {
			RowRecord[] oldRows = rows;
			rows = new RowRecord[Math
					.max(oldRows.length + rowGrowSize, row + 1)];
	...
	/**
	 * Sets the width (in characters) and format options for a particular column
	 * in this sheet
	 * 
	 * @param col
	 *            the column to set
	 ...
	 * @param format
	 *            the formt details for the column
	 */
	 ...
	 *
	 * @param col
	 *            the column on which to set the view
	 * @param view
	 *            the view to set
	 */
	public void setColumnView(int col, CellView view) {
		...
		try {
			...
			int width = view.depUsed() ? view.getDimension() * 256 : view
					.getSize();
			...
			ColumnInfoRecord cir = new ColumnInfoRecord(col,
					view.getDimension() * 256, WritableWorkbook.NORMAL_STYLE);
			if (!columnFormats.contains(cir)) {
				columnFormats.add(cir);
			}
		}
	 ...
	 * @param height
	 *            the row height in 1/20ths of a point
	 * @exception RowsExceededException
	 ...
	 *            the row to be formatted
	 * @param height
	 *            the row height in 1/20th of a point
	 * @param collapsed
	 *            indicates whether the row is collapsed
	 * @param zeroHeight
	 *            indicates that the row has zero height
	 * @exception RowsExceededException
	 * @deprecated use the override which takes a CellView object
	 */
	...
	public void setRowView(int row, CellView view) throws RowsExceededException {
		...
		rowrec.setRowDetails(view.getSize(), false, view.isHidden(), 0, false,
				xfr);
	...
	public void write() throws IOException {
		...
		sheetWriter.setWriteData(rows, rowBreaks, columnBreaks, hyperlinks,
				mergedCells, columnFormats, maxRowOutlineLevel,
	...
	/**
	 * Removes the specified hyperlink. Note that if you merely set the cell
	 * contents to be an Empty cell, then the cells containing the hyperlink
	 * will still be active. The contents of the cell which activate the
	 * hyperlink are removed. The hyperlink passed in must be a hyperlink
	 * retrieved using the getHyperlinks method
	 *
	 * @param h
	 ...
	 * Removes the specified hyperlink. Note that if you merely set the cell
	 * contents to be an Empty cell, then the cells containing the hyperlink
	 * will still be active. If the preserveLabel field is set, the cell
	 * contents of the hyperlink are preserved, although the hyperlink is
	 * deactivated. If this value is FALSE, the cell contents are removed The
	 * hyperlink passed in must be a hyperlink retrieved using the getHyperlinks
	 * method
	 *
	 * @param h
	 *            the hyperlink to remove.
	...
	public void removeHyperlink(WritableHyperlink h, boolean preserveLabel) {
		...
		if (!preserveLabel) {
			// Set the cell contents for the hyperlink - including any
			// formatting
	 ...
	 * @exception RowsExceededException
	 */
	public void addHyperlink(WritableHyperlink h) throws WriteException,
			RowsExceededException {
	...
	/**
	 * Merges the specified cells. Any clashes or intersections between merged
	 * cells are resolved when the spreadsheet is written out
	 *
	 * @param col1
	 ...
	 *
	 * @param row1
	 *            the first row to unset
	 * @param row2
	 *            the last row to unset
	 * @exception WriteException
	 * @exception RowsExceededException
	 */
	public void unsetRowGroup(int row1, int row2) throws WriteException,
			RowsExceededException {
		if (row2 < row1) {
			logger.warn("Cannot merge cells - top and bottom rows incorrectly "
					+ "specified");
	 ...
	 *
	 * @param col1
	 *            the first column to unset
	 * @param col2
	 *            the last column to unset
	 * @exception WriteException
	 * @exception RowsExceededException
	 */
	public void unsetColumnGroup(int col1, int col2) throws WriteException,
			RowsExceededException {
		if (col2 < col1) {
			logger.warn("Cannot merge cells - top and bottom rows incorrectly "
					+ "specified");
	...
	/**
	 * Unmerges the specified cells. The Range passed in should be one that has
	 * been previously returned as a result of the getMergedCells method
	 *
	 * @param r
	 ...
	 *
	 * @param l
	 *            the print header to print on the left side
	 * @param c
	 *            the print header to print in the centre
	 * @param r
	 ...
	 * @deprecated Use the SheetSettings bean
	 */
	public void setPageSetup(PageOrientation p, PaperSize ps, double hm,
			double fm) {
	...
	/**
	 * Check all the merged cells for borders. Although in an OO sense the logic
	 * should belong in this class, in order to reduce the bloated nature of the
	 * source code for this object this logic has been delegated to the
	 * SheetWriter
	 */
	 ...
	 * 
	 * @param xfMapping
	 *            the index mapping for XFRecords
	 * @param fontMapping
	 *            the index mapping for fonts
	 * @param formatMapping
	 *            the index mapping for formats
	 */
	void rationalize(IndexMapping xfMapping, IndexMapping fontMapping,
	...
	public void addImage(WritableImage image) {
		...
		if (imageFile != null) {
			...
			fileType = fileTypeIndex != -1 ? fileName
	 ...
	 *
	 * @param i
	 *            the 0-based image index number
	...
	/**
	 * Removes the specified image from this sheet. The image passed in must be
	 * the same instance as that retrieved from a getImage call
	 *
	 * @param wi
	 *            the image to remove
	 */
	...
	/**
	 * Removes the data validation for the specified cell. Called from CellValue
	 * in response to a cell being replaced
	 *
	 * @param cv
	 *            the cell being removed
	 */
	...
	private void autosizeColumn(int col) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			if (cell != null) {
				...
				if (activeFont.isItalic() || activeFont.getBoldWeight() > 400) // magic
																				// value
																				// for
																				// normal
																				// bold
	 ...
	 *
	 * @param c
	 *            the number of cells accross to apply this data validation
	 * @param r
	 *            the number of cells downwards to apply this data validation
	 */
	public void applySharedDataValidation(WritableCell c, int extraCols,
		...
		if (c.getWritableCellFeatures() == null
			...
			logger.warn("Cannot extend data validation for "
					+ CellReferenceHelper.getCellReference(c.getColumn(),
							c.getRow()) + " as it has no data validation");
			return;
		}
		...
		for (int y = startRow; y <= endRow; y++) {
			if (rows[y] != null) {
				int endCol = Math.min(rows[y].getMaxColumn() - 1, startColumn
						+ extraCols);
				for (int x = startColumn; x <= endCol; x++) {
					// Ignore the first cell
					if (x == startColumn && y == startRow) {
						continue; // continue statements - they're no better
									// than gotos
					}

					WritableCell c2 = rows[y].getCell(x);

					// Check that the target cell does not have any data
					// validation
					if (c2 != null && c2.getWritableCellFeatures() != null
						...
						logger.warn("Cannot apply data validation from "
								...
								+ " to "
								+ CellReferenceHelper.getCellReference(
										startColumn + extraCols, startRow
												+ extraRows) + " as cell "
		...
		for (int y = startRow; y <= startRow + extraRows; y++) {
			RowRecord rowrec = getRowRecord(y); // create the row if it doesn't
												// exist
			for (int x = startColumn; x <= startColumn + extraCols; x++) {
				// Ignore the first cell
				if (x == startColumn && y == startRow) {
					continue; // continue statements - they're no better than
								// gotos
				}
				...
				if (c2 == null) {
					...
					WritableCellFeatures validation = c2
	...
	/**
	 * Remove the shared data validation from multiple cells. The cell passed in
	 * is the top left cell. The data validation is removed from this cell and
	 * all cells which share the same validation.
	 *
	 * @param cell
	 *            the top left cell containing the shared data validation
	 */
	public void removeSharedDataValidation(WritableCell cell)
		...
		// Check that the cell validation being removed is in the top left of
		// the
		// validated area
		if (dvp.extendedCellsValidation()) {
			if (cell.getColumn() != dvp.getFirstColumn()
				...
				logger.warn("Cannot remove data validation from "
						...
						+ "-"
						+ CellReferenceHelper.getCellReference(
								dvp.getLastColumn(), dvp.getLastRow())
		...
		if (dataValidation != null) {
			dataValidation.removeSharedDataValidation(dvp.getFirstColumn(),
					dvp.getFirstRow(), dvp.getLastColumn(), dvp.getLastRow());
		}
	}
}
