 ...
 * A writable workbook
 */
public class WritableWorkbookImpl extends WritableWorkbook implements
	...
	/**
	 * Indicates whether or not the output stream should be closed. This depends
	 * on whether this Workbook was created with an output stream, or a flat
	 * file (flat file closes the stream
	 */
	 ...
	 *            the output stream
	 * @param cs
	 *            TRUE if the workbook should close the output stream, FALSE
	 * @param ws
	 *            the configuration for this workbook otherwise
	 */
	 ...
	 */
	public WritableWorkbookImpl(OutputStream os, Workbook w, boolean cs,
	...
	/**
	 * Interface method from WorkbookMethods - gets the specified sheet within
	 * this workbook
	 *
	 * @param index
	...
	/**
	 * Sets a new output file. This allows the smae workbook to be written to
	 ...
	 * @param index
	 * @param handleRefs
	 *            flag indicating whether or not to handle external sheet
	 *            references
	 * @return
	 */
	private WritableSheet createSheet(String name, int index, boolean handleRefs) {
		WritableSheet w = new WritableSheetImpl(name, outputFile,
				formatRecords, sharedStrings, settings, this);
	...
	/**
	 * Creates a new sheet within the workbook, at the specified position. The
	 * new sheet is inserted at the specified position, or prepended/appended to
	 * the list of sheets if the index specified is somehow inappropriate
	 * 
	 * @param name
	 ...
	 * altered accordingly. If the sheet referenced by the index does not exist,
	...
	/**
	 * Writes out this sheet to the output file. First it writes out the
	 * standard workbook information required by excel, before calling the write
	 * method on each sheet individually
	 * 
	 * @exception IOException
	 */
	public void write() throws IOException {
		...
		for (int i = 0; i < getNumberOfSheets(); i++) {
			...
			// globals are written out because some more XF formats might be
			// created
			...
			if (range != null) {
				addNameArea(BuiltInName.PRINT_AREA, wsi, range.getTopLeft()
						.getColumn(), range.getTopLeft().getRow(), range
						.getBottomRight().getColumn(), range.getBottomRight()
						.getRow(), false);
			}
			...
			if (rangeR != null && rangeC != null) {
				addNameArea(BuiltInName.PRINT_TITLES, wsi, rangeR.getTopLeft()
						.getColumn(), rangeR.getTopLeft().getRow(), rangeR
						.getBottomRight().getColumn(), rangeR.getBottomRight()
						.getRow(), rangeC.getTopLeft().getColumn(), rangeC
						.getTopLeft().getRow(), rangeC.getBottomRight()
						.getColumn(), rangeC.getBottomRight().getRow(), false);
			}
			// Check to see if print titles by row were set
			else if (rangeR != null) {
				addNameArea(BuiltInName.PRINT_TITLES, wsi, rangeR.getTopLeft()
			...
			// Check to see if print titles by column were set
				...
				addNameArea(BuiltInName.PRINT_TITLES, wsi, rangeC.getTopLeft()
						.getColumn(), rangeC.getTopLeft().getRow(), rangeC
						.getBottomRight().getColumn(), rangeC.getBottomRight()
						.getRow(), false);
			}
		}
		...
		if (addInFunctionNames != null && addInFunctionNames.length > 0) {
			...
			for (int i = 0; i < addInFunctionNames.length; i++) {
				ExternalNameRecord enr = new ExternalNameRecord(
						addInFunctionNames[i]);
	...
	/**
	 * Produces a writable copy of the workbook passed in by creating copies of
	 * each sheet in the specified workbook and adding them to its own record
	 * 
	 * @param w
	 *            the workbook to copy
	 */
	...
	/**
	 * Parsing of formulas is only supported for a subset of the available biff
	 * version, so we need to test to see if this version is acceptable
	 *
	...
	public int getExternalSheetIndex(String sheetName) {
		...
		if (found) {
			...
			if (supbook.getType() != SupbookRecord.INTERNAL
				...
				logger.warn("Cannot find sheet " + sheetName
						+ " in supbook record");
			}
		...
		String worksheetName = sheetName
				.substring(closeSquareBracketsIndex + 1);
	 ...
	 *
	 * @param s
	 *            the sheet on which the column was inserted
	 * @param col
	 *            the column number which was inserted
	 */
	...
	/**
	 * Called when a column is removed on the specified sheet. Notifies all RCIR
	 * cells of this change
	 *
	 * @param s
	 *            the sheet on which the column was removed
	 * @param col
	 *            the column number which was removed
	 */
	void columnRemoved(WritableSheetImpl s, int col) {
		...
		if (names != null) {
			for (Iterator i = names.iterator(); i.hasNext();) {
				NameRecord nameRecord = (NameRecord) i.next();
				boolean removeName = nameRecord.columnRemoved(
						externalSheetIndex, col);
			...
			for (Iterator i = removedNames.iterator(); i.hasNext();) {
				...
				Assert.verify(removed,
						"Could not remove name " + nameRecord.getName());
			}
		}
	}

	/**
	 * Called when a row is inserted on the specified sheet. Notifies all RCIR
	 * cells of this change
	 *
	 * @param s
	 *            the sheet on which the row was inserted
	 * @param row
	 *            the row number which was inserted
	 */
	...
	/**
	 * Called when a row is removed on the specified sheet. Notifies all RCIR
	 * cells of this change
	 *
	 * @param s
	 *            the sheet on which the row was removed
	 * @param row
	 *            the row number which was removed
	 */
	void rowRemoved(WritableSheetImpl s, int row) {
		...
		if (names != null) {
			for (Iterator i = names.iterator(); i.hasNext();) {
				NameRecord nameRecord = (NameRecord) i.next();
				boolean removeName = nameRecord.rowRemoved(externalSheetIndex,
						row);
	 ...
	 *
	 * @param the
	 *            name of the cell/range to search for
	...
	public Range[] findByName(String name) {
		...
		for (int i = 0; i < ranges.length; i++) {
			cellRanges[i] = new RangeImpl(this,
					...
					ranges[i].getFirstColumn(), ranges[i].getFirstRow(),
					getLastExternalSheetIndex(ranges[i].getExternalSheet()),
					ranges[i].getLastColumn(), ranges[i].getLastRow());
		}
	 ...
	 * which contain drawings, but this workbook doesn't. We can't subsume this
	 * into the getDrawingGroup() method because the null-ness of the return
	 * value is used elsewhere to determine the origin of the workbook
	 */
	 ...
	 */
	public void addNameArea(String name, WritableSheet sheet, int firstCol,
			int firstRow, int lastCol, int lastRow) {
	 ...
	 *            last row this name refers to.
	 * @param global
	 *            TRUE if this is a global name, FALSE if this is tied to the
	 *            sheet
	 */
	void addNameArea(String name, WritableSheet sheet, int firstCol,
			int firstRow, int lastCol, int lastRow, boolean global) {
		if (names == null) {
			names = new ArrayList();
		}
		...
		NameRecord nr = new NameRecord(name, names.size(), externalSheetIndex,
				firstRow, lastRow, firstCol, lastCol, global);
	 ...
	 *            sheet
	 */
	void addNameArea(BuiltInName name, WritableSheet sheet, int firstCol,
			int firstRow, int lastCol, int lastRow, boolean global) {
		if (names == null) {
			names = new ArrayList();
		}
		...
		// Create a new name record.
		NameRecord nr = new NameRecord(name, index, externalSheetIndex,
				firstRow, lastRow, firstCol, lastCol, global);
	 ...
	 */
	void addNameArea(BuiltInName name, WritableSheet sheet, int firstCol,
			int firstRow, int lastCol, int lastRow, int firstCol2,
			int firstRow2, int lastCol2, int lastRow2, boolean global) {
		if (names == null) {
			names = new ArrayList();
		}
		...
		// Create a new name record.
		NameRecord nr = new NameRecord(name, index, externalSheetIndex,
				firstRow2, lastRow2, firstCol2, lastCol2, firstRow, lastRow,
				firstCol, lastCol, global);
