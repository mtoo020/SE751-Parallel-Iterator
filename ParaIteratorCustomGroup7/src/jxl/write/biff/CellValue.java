...
/**
 * Abstract class which stores the jxl.common.data used for cells, such as row,
 * column and formatting information. Any record which directly represents the
 * contents of a cell, such as labels and numbers, are derived from this class
 * data store
 */
public abstract class CellValue extends WritableRecordData implements
		WritableCell {
	/**
	 * The logger
	 */
	...
	/**
	 * A handle to the formatting records, used in case we want to change the
	 * format of the cell once it has been added to the spreadsheet
	 */
	private FormattingRecords formattingRecords;

	/**
	 * A flag to indicate that this record is already referenced within a
	 * worksheet
	 */
	...
	/**
	 * Internal copied flag, to prevent cell features being added multiple times
	 * to the drawing array
	 */
	...
	/**
	 * Overloaded constructor used when building writable cells from the Java
	 * API which also takes a format
	 * 
	 * @param c
	 *            the column
	 * @param t
	 ...
	 *            the row
	 * @param st
	 *            the format to apply to this cell
	 */
	...
	/**
	 * Indicates whether or not this cell is hidden, by virtue of either the
	 * entire row or column being collapsed
	 *
	...
	/**
	 * Called when the cell is added to the worksheet in order to indicate that
	 * this object is already added to the worksheet This method also verifies
	 * that the associated formats and formats have been initialized correctly
	 * 
	 * @param fr
	 *            the formatting records
	 * @param ss
	 *            the shared strings used within the workbook
	 * @param s
	 ...
	 * @param sheetIndex
	 *            the sheet index on which the column was inserted
	 * @param col
	 ...
	 * cells of this change. The default implementation here does nothing
	 *
	 * @param s
	 *            the sheet on which the column was inserted
	 * @param sheetIndex
	 ...
	 * @param sheetIndex
	 *            the sheet index on which the column was removed
	 * @param row
	 ...
	 * checks (and clones) to ensure that the formats are not shared. Called
	 * from setCellDetails and setCellFormat
	 */
	...
	public void setCellFeatures(WritableCellFeatures cf) {
		if (features != null) {
			...
			if (features.hasDataValidation() && features.getDVParser() != null
	 ...
	 * Called internally from this class when a cell is added to the workbook,
	 * and also externally from BaseCellFeatures following a call to setComment
	 */
	public final void addCellFeatures() {
		...
		if (features.hasDataValidation()) {
			try {
				features.getDVParser().setCell(column, row,
						sheet.getWorkbook(), sheet.getWorkbook(),
			...
			if (sheet.getComboBox() == null) {
				...
				// it doesn't need a separate Sp entry, it still needs to
				// increment
				// the shape id
	...
	public final void removeCellFeatures() {
		/*
		 * // Remove the comment features.removeComment();
		 * 
		 * // Remove the data validation features.removeDataValidation();
		 */
	...
	/**
	 * Called when doing a copy of a writable object to indicate the source was
	 * writable than a read only copy and certain things (most notably the
	 * comments will need to be re-evaluated)
	 *
