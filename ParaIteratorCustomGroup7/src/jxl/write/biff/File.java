...
/**
 * A file of excel data to be written out. All the excel data is held in memory,
 * and when the close method is called a CompoundFile object is used to write
 * the Biff oriented excel data in the CompoundFile format
 */
public final class File {
	...
	/**
	 * The read compound file. This will only be non-null if there are macros or
	 * other property sets of that ilk which that we should be copying
	 */
	 ...
	 *            the output stream
	 * @param ws
	 *            the configuration settings for this workbook
	 * @param rcf
	 *            the rea compound file
	 */
	...
	/**
	 * Closes the file. In fact, this writes out all the excel data to disk
	 * using a CompoundFile object, and then frees up all the memory allocated
	 * to the workbook
	 * 
	...
	void close(boolean cs) throws IOException, JxlWriteException {
		CompoundFile cf = new CompoundFile(data, data.getPosition(),
				outputStream, readCompoundFile);
		cf.write();
	 ...
	 * @exception IOException
	 * @param record
	 *            the record to add to the excel data
	 */
	...
	/**
	 * Used to manually alter the contents of the written out data. This is used
	 * when cross-referencing cell records
	 * 
	 * @param pos
