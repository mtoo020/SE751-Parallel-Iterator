...
public class HyperlinkRecord extends WritableRecordData {
	...
	/**
	 * The range of this hyperlink. When creating a hyperlink, this will be null
	 * until the hyperlink is added to the sheet
	 */
	private Range range;
	 ...
	 *
	 * @param hl
	 *            the hyperlink from the read spreadsheet
	 */
	...
	private void copyReadHyperlink(Hyperlink h, WritableSheet s) {
		...
		range = new SheetRangeImpl(s, firstColumn, firstRow, lastColumn,
				lastRow);
	...
	/**
	 * Copies a hyperlink read in from a writable sheet. Used when copying
	 * writable sheets
	 *
	 * @param hl
	 *            the hyperlink from the read spreadsheet
	 ...
	 *
	 * @param col
	 *            the column containing this hyperlink
	 * @param row
	 *            the row containing this hyperlink
	 * @param lastcol
	 *            the last column which activates this hyperlink
	 * @param lastrow
	 *            the last row which activates this hyperlink
	 * @param url
	 *            the hyperlink
	 * @param desc
	 *            the description
	 ...
	 *            the last row which activates this hyperlink
	 * @param desc
	 *            the contents of the cell which describe this hyperlink
	 * @param sheet
	 *            the sheet containing the cells to be linked to
	 * @param destcol
	 *            the column number of the first destination linked cell
	 * @param destrow
	 *            the row number of the first destination linked cell
	 * @param lastdestcol
	 *            the column number of the last destination linked cell
	 * @param lastdestrow
	 *            the row number of the last destination linked cell
	 */
	protected HyperlinkRecord(int col, int row, int lastcol, int lastrow,
			String desc, WritableSheet s, int destcol, int destrow,
	...
	/**
	 * Gets the range of cells which activate this hyperlink The get sheet index
	 * methods will all return -1, because the cells will all be present on the
	 * same sheet
	 *
	 * @return the range of cells which activate the hyperlink or NULL if this
	 *         hyperlink has not been added to the sheet
	 */
	...
	public void setURL(URL url) {
		...
		// Change the label on the sheet if it was a string representation of
		// the
		// URL
	 ...
	 *            the row number of the last destination linked cell
	 */
	protected void setLocation(String desc, WritableSheet sheet, int destcol,
	 ...
	 *            the row number of the last destination linked cell
	 */
	private void setLocation(WritableSheet sheet, int destcol, int destrow,
			int lastdestcol, int lastdestrow) {
	 ...
	 *
	 * @param r
	 *            the row which has been inserted
	 */
	void insertRow(int r) {
		...
		if (modified) {
			range = new SheetRangeImpl(sheet, firstColumn, firstRow,
					lastColumn, lastRow);
		}
	}
	 ...
	 *
	 * @param c
	 *            the column which has been inserted
	 */
	 ...
	 *
	 * @param c
	 *            the column which has been removed
	 */
	 ...
	 *
	 * @param cd
	 *            the data jxl.common.for all types of hyperlink
	...
	private byte[] getURLData(byte[] cd) {
		...
		IntegerHelper
				.getFourBytes((urlString.length() + 1) * 2, d, urlPos + 16);
	...
	private byte[] getFileData(byte[] cd) {
		...
		int dataLength = cd.length + 4 + (shortFilePath.length() + 1) + // short
																		// file
																		// name
		...
		/*
		 * curPos += 24; int nameLength = filePath.length() * 2;
		 * 
		 * // Size of the file link IntegerHelper.getFourBytes(nameLength+6, d,
		 * curPos);
		 * 
		 * // Number of characters IntegerHelper.getFourBytes(nameLength, d,
		 * curPos+4);
		 * 
		 * // Inexplicable byte sequence d[curPos+8] = 0x03;
		 * 
		 * // The long file name StringHelper.getUnicodeBytes(filePath, d,
		 * curPos+10);
		 */
