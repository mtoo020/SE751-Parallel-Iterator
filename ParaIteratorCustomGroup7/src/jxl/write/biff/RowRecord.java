...
class RowRecord extends WritableRecordData {
	...
	/**
	 * Sets the row details based upon the readable row record passed in Called
	 * when copying spreadsheets
	 *
	 * @param height
	 *            the height of the row record in 1/20ths of a point
	 * @param mdfh
	 *            matches the default font height
	 * @param col
	 ...
	 *            the group start
	 * @param xf
	 *            the xfrecord for the row (NULL if no default is set)
	 */
	void setRowDetails(int height, boolean mdfh, boolean col, int ol,
			boolean gs, XFRecord xfr) {
		rowHeight = height;
		collapsed = col;
	...
	public void addCell(CellValue cv) {
		...
		if (cells[col] != null) {
			...
			if (wcf != null) {
				...
				// if the cell is part of a shared data validation,then don't
				// remove
				// the validation
	 ...
	 * 
	 * @param col
	 *            the column at which to remove the cell
	 */
	...
	/**
	 * Writes out all the cells in this row. If more than three integer values
	 * occur consecutively, then a MulRK record is used to group the numbers
	 * 
	 * @exception IOException
	 * @param outputFile
	 *            the output file
	...
	public void writeCells(File outputFile) throws IOException {
		...
		for (int i = 0; i < numColumns; i++) {
			integerValue = false;
			if (cells[i] != null) {
				// See if this cell is a 30-bit integer value (without
				// additional
				// cell features)
				...
				if (integerValue) {
					...
					// This cell is not an integer. Write out whatever integers
					// we
					// have, and then write out this cell
					writeIntegerValues(integerValues, outputFile);
					outputFile.write(cells[i]);

					// If the cell is a string formula, write out the string
					// record
	...
	/**
	 * Writes out the list of integer values. If there are more than three, a
	 * MulRK record is used, otherwise a sequence of Numbers is used
	 * 
	 * @exception IOException
	 * @param outputFile
	 *            the output file
	 ...
	 * 
	 * @param col
	 *            the colun for which to return the cell
	 ...
	 * level. If all outline levels are gone the uncollapse the row.
	 */
