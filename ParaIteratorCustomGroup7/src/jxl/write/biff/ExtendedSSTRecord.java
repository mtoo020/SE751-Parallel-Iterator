...
/**
 * Indicates an extension to the Shared String Table. Currently this contains
 * blank records
 *
...
class ExtendedSSTRecord extends WritableRecordData {
	 ...
	 *
	 * @param numstrings
	 *            the number of strings per bucket
	 * @param streampos
	 *            the absolute stream position of the beginning of the SST
	 *            record
	 */
	...
	public int getNumberOfBuckets() {
		...
		return numberOfStringsPerBucket != 0 ? (numberOfStrings
				+ numberOfStringsPerBucket - 1)
	...
	public void addString(int absoluteStreamPosition, int relativeStreamPosition) {
		absoluteStreamPositions[currentStringIndex] = absoluteStreamPosition
				+ relativeStreamPosition;
	...
	public byte[] getData() {
		...
		for (int i = 0; i < numberOfBuckets; i++) {
			...
			IntegerHelper.getFourBytes(absoluteStreamPositions[i], data,
					2 + (i * infoRecordSize));
			// relative offset
			IntegerHelper.getTwoBytes(relativeStreamPositions[i], data,
					6 + (i * infoRecordSize));
			// reserved
