...
/**
 * Contains the functionality necessary for writing out a sheet. Originally this
 * was incorporated in WritableSheetImpl, but was moved out into a dedicated
 * class in order to reduce the over bloated nature of that class
 */
final class SheetWriter {
	...
	/**
	 * A handle back to the writable sheet, in order for this class to invoke
	 * the get accessor methods
	 */
	...
	/**
	 * Writes out this sheet. First writes out the standard sheet information
	 * then writes out each row in turn. Once all the rows have been written
	 * out, it retrospectively adjusts the offset references in the file
	 * 
	...
	public void write() throws IOException {
		...
		if (settings.isProtected()) {
			...
			if (settings.getPassword() != null) {
				...
				PasswordRecord pw = new PasswordRecord(
		...
		DefaultColumnWidth dcw = new DefaultColumnWidth(
				settings.getDefaultColumnWidth());
		outputFile.write(dcw);

		// Get a handle to the normal styles
		WritableCellFormat normalStyle = sheet.getWorkbook().getStyles()
				.getNormalStyle();
		WritableCellFormat defaultDateFormat = sheet.getWorkbook().getStyles()
		...
		for (Iterator colit = columnFormats.iterator(); colit.hasNext();) {
			...
			// Writing out the column info with index 0x100 causes excel to
			// crash
			...
			if (xfr != normalStyle && cir.getColumn() < 0x100) {
				...
				for (int i = 0; i < cells.length; i++) {
					if (cells[i] != null
							&& (cells[i].getCellFormat() == normalStyle || cells[i]
									.getCellFormat() == defaultDateFormat)) {
		...
		if (settings.getHorizontalFreeze() != 0
			...
			// Handle the selection record. First, there will always be a top
			// left
			SelectionRecord sr = new SelectionRecord(SelectionRecord.upperLeft,
					0, 0);
			outputFile.write(sr);

			// Top right
			if (settings.getHorizontalFreeze() != 0) {
				sr = new SelectionRecord(SelectionRecord.upperRight,
						settings.getHorizontalFreeze(), 0);
				outputFile.write(sr);
			}

			// Bottom left
			if (settings.getVerticalFreeze() != 0) {
				sr = new SelectionRecord(SelectionRecord.lowerLeft, 0,
						settings.getVerticalFreeze());
				outputFile.write(sr);
			}
			...
			if (settings.getHorizontalFreeze() != 0
					&& settings.getVerticalFreeze() != 0) {
				sr = new SelectionRecord(SelectionRecord.lowerRight,
	...
	/**
	 * Sets the data necessary for writing out the sheet. This method must be
	 * called immediately prior to writing
	 *
	 * @param rws
	 *            the rows in the spreadsheet
	 */
	void setWriteData(RowRecord[] rws, ArrayList rb, ArrayList cb,
			ArrayList hl, MergedCells mc, TreeSet cf, int mrol, int mcol) {
		rows = rws;
		rowBreaks = rb;
		columnBreaks = cb;
		hyperlinks = hl;
	...
	/**
	 * Sets the sheet settings for this particular sheet. Must be called
	 * immediately prior to writing
	 * 
	 * @param sr
	 *            the sheet settings
	...
	/**
	 * Check all the merged cells for borders. If the merge record has borders,
	 * then we need to rejig the cell formats to take account of this. This is
	 * called by the write method of the WritableWorkbookImpl, so that any new
	 * XFRecords that are created may be written out with the others
	 */
	void checkMergedBorders() {
		...
		for (int mci = 0; mci < mcells.length; mci++) {
			...
			if (tlformat != null && tlformat.hasBorders() == true
					&& !tlformat.isRead()) {
				try {
					...
					if (bottomRight.getRow() > topLeft.getRow()) {
						...
						// Handle the cells down the left hand side (and along
						// the
						// right too, if necessary)
						for (int i = topLeft.getRow() + 1; i < bottomRight
								.getRow(); i++) {
					...
					if (bottomRight.getColumn() > topLeft.getColumn()) {
						...
						for (int i = topLeft.getRow() + 1; i < bottomRight
							...
							sheet.addCell(new Blank(bottomRight.getColumn(), i,
									cf7));
						}

						// Handle the cells along the top, and along the bottom
						// too
						for (int i = topLeft.getColumn() + 1; i < bottomRight
								.getColumn(); i++) {
					...
					if (bottomRight.getColumn() > topLeft.getColumn()
						...
						for (int i = topLeft.getColumn() + 1; i < bottomRight
							...
							sheet.addCell(new Blank(i, bottomRight.getRow(),
									cf5));
						}
					}
	...
	/**
	 * Get the cells in the column. Don't use the interface method getColumn for
	 * this as this will create loads of empty cells, and we could do without
	 * that overhead
	 */
	 ...
	 *
	 * @param dv
	 *            the read-in list of data validations
	 * @param vc
	 *            the api manipulated set of data validations
	 */
	...
	private void writeDataValidation() throws IOException {
		...
		if (dataValidation == null && validatedCells.size() > 0) {
			...
			int comboBoxId = sheet.getComboBox() != null ? sheet.getComboBox()
					.getObjectId() : DataValidation.DEFAULT_OBJECT_ID;
			dataValidation = new DataValidation(comboBoxId,
					sheet.getWorkbook(), sheet.getWorkbook(), workbookSettings);
		}
		...
		for (Iterator i = validatedCells.iterator(); i.hasNext();) {
			...
			if (!cf.getDVParser().copied()) {
				if (!cf.getDVParser().extendedCellsValidation()) {
					// DVParser is specific for a single cell validation - just
					// add it
	...
	/*
	 * 
	 * // There is a mixture of read and write validations for (Iterator i =
	 * validatedCells.iterator(); i.hasNext(); ) { CellValue cv = (CellValue)
	 * i.next(); CellFeatures cf = cv.getCellFeatures();
	 * DataValiditySettingsRecord dvsr = new
	 * DataValiditySettingsRecord(cf.getDVParser()); dataValidation.add(dvsr); }
	 * dataValidation.write(outputFile); return; }
	 */
}
