...
/**
 * A structure containing the styles used by this workbook. This is used to
 * enforce thread safety by tying the default styles to a workbook instance
 * rather than by initializing them statically
 */
class Styles {
	/**
	 * The logger
	 ...
	 * is already thread safe (ie. it doesn't use a statically initialized
	 * format or font) then the same object is simply returned This object is
	 * already tied to a workbook instance, so no synchronisation is necesasry
	 *
	 * @param wf
	 *            a format to verify
