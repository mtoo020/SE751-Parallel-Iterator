 ...
 * 
 * Header block is -1 Excel data is e..n (where e is the head extension blocks,
 * normally 0 and n is at least 8) Summary information (8 blocks) Document
 * summary (8 blocks) BBD is block p..q (where p=e+n+16 and q-p+1 is the number
 * of BBD blocks) Property storage block is q+b...r (normally 1 block) (where b
 * is the number of BBD blocks)
 */
final class CompoundFile extends BaseCompoundFile {
	...
	/**
	 * The size the excel data should be in order to comply with the general
	...
	/**
	 * The number of extension blocks required for the header to describe the
	 * BBD
	 */
	...
	/**
	 * The number of blocks required to store the root entry property sets and
	 * small block depot
	 */
	...
	public CompoundFile(ExcelDataOutput data, int l, OutputStream os,
		...
		numPropertySets = 4 + (additionalPropertySets != null ? additionalPropertySets
				.size() : 0);

		if (additionalPropertySets != null) {
			numSmallBlockDepotChainBlocks = getBigBlocksRequired(numSmallBlocks * 4);
			numSmallBlockDepotBlocks = getBigBlocksRequired(numSmallBlocks
					* SMALL_BLOCK_SIZE);

			numRootEntryBlocks += getBigBlocksRequired(additionalPropertySets
					.size() * PROPERTY_STORAGE_BLOCK_SIZE);
		}
		...
		int startTotalBlocks = excelDataBlocks + 8
				+ // summary block
				8
				+ // document information
				additionalPropertyBlocks + numSmallBlockDepotBlocks
				+ numSmallBlockDepotChainBlocks + numRootEntryBlocks;
		...
		if (numBigBlockDepotBlocks > blockChainLength - 1) {
			...
			totalBlocks = startTotalBlocks + numExtensionBlocks
		...
		if (additionalPropertySets != null && numSmallBlockDepotBlocks != 0) {
			sbdStartBlock = excelDataStartBlock + excelDataBlocks
					+ additionalPropertyBlocks + 16;
		}
		...
		if (sbdStartBlockChain != -2) {
			...
			bbdStartBlock = excelDataStartBlock + excelDataBlocks
					+ additionalPropertyBlocks + 16;
		}
		...
		if (totalBlocks != rootStartBlock + numRootEntryBlocks) {
			...
			logger.warn("RootStartBlock " + rootStartBlock + " totalBlocks "
					+ totalBlocks);
		}
	}
		...
		for (int i = 0; i < numPropertySets; i++) {
			...
			for (int j = 0; j < STANDARD_PROPERTY_SETS.length && !standard; j++) {
				if (ps.name.equalsIgnoreCase(STANDARD_PROPERTY_SETS[j])) {
					...
					PropertyStorage ps2 = readCompoundFile
							.findPropertyStorage(ps.name);
					Assert.verify(ps2 != null);

					if (ps2 == ps) {
						standard = true;
						ReadPropertyStorage rps = new ReadPropertyStorage(ps,
								null, i);
						standardPropertySets
								.put(STANDARD_PROPERTY_SETS[j], rps);
					}
				}
			}
			...
			if (!standard) {
				try {
					...
					ReadPropertyStorage rps = new ReadPropertyStorage(ps, data,
							i);
	...
	public void write() throws IOException {
		...
		// Don't flush or close the stream - this is handled by the enclosing
		// File
		// object
	}
	...
	private void writeHeader() throws IOException {
		...
		byte[] extensionBlockData = new byte[BIG_BLOCK_SIZE
				* numExtensionBlocks];
		...
		IntegerHelper.getFourBytes(numBigBlockDepotBlocks, headerBlock,
				NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);

		// Set the small block depot chain
		IntegerHelper.getFourBytes(sbdStartBlockChain, headerBlock,
				SMALL_BLOCK_DEPOT_BLOCK_POS);

		// Set the number of blocks in the small block depot chain
		IntegerHelper.getFourBytes(numSmallBlockDepotChainBlocks, headerBlock,
				NUM_SMALL_BLOCK_DEPOT_BLOCKS_POS);

		// Set the extension block
		IntegerHelper.getFourBytes(extensionBlock, headerBlock,
				EXTENSION_BLOCK_POS);

		// Set the number of extension blocks to be the number of BBD blocks - 1
		IntegerHelper.getFourBytes(numExtensionBlocks, headerBlock,
				NUM_EXTENSION_BLOCK_POS);

		// Set the root start block
		IntegerHelper.getFourBytes(rootStartBlock, headerBlock,
		...
		int blocksToWrite = Math.min(numBigBlockDepotBlocks,
				(BIG_BLOCK_SIZE - BIG_BLOCK_DEPOT_BLOCKS_POS) / 4);
		int blocksWritten = 0;
		...
		for (int extBlock = 0; extBlock < numExtensionBlocks; extBlock++) {
			...
			int nextBlock = (blocksWritten == numBigBlockDepotBlocks) ? -2
					: extBlock + 1;
	...
	/**
	 * Checks that the data can fit into the current BBD block. If not, then it
	 ...
	 *
	 * @param startBlock
	 *            the starting block of the big block chain
	 * @param numBlocks
	...
	private void writeBlockChain(int startBlock, int numBlocks)
		...
		while (blocksToWrite > 0) {
			int bbdBlocks = Math.min(blocksToWrite,
					(BIG_BLOCK_SIZE - bbdPos) / 4);
	...
	private void writeSmallBlockDepotChain() throws IOException {
		...
		byte[] smallBlockDepotChain = new byte[numSmallBlockDepotChainBlocks
				* BIG_BLOCK_SIZE];
	...
	private void writeSmallBlockDepot() throws IOException {
		...
		byte[] smallBlockDepot = new byte[numSmallBlockDepotBlocks
				* BIG_BLOCK_SIZE];
		...
		for (Iterator i = additionalPropertySets.iterator(); i.hasNext();) {
			...
			if (rps.data.length <= SMALL_BLOCK_THRESHOLD) {
				...
				System.arraycopy(rps.data, 0, smallBlockDepot, pos,
						rps.data.length);
				pos += length;
			}
		}
	...
	private void writePropertySets() throws IOException {
		...
		if (additionalPropertySets != null) {
			...
			for (int i = 0; i < STANDARD_PROPERTY_SETS.length; i++) {
				ReadPropertyStorage rps = (ReadPropertyStorage) standardPropertySets
						.get(STANDARD_PROPERTY_SETS[i]);

				if (rps != null) {
					mappings[rps.number] = i;
				} else {
					logger.warn("Standard property set "
							+ STANDARD_PROPERTY_SETS[i]
		...
		if (additionalPropertySets != null) {
			...
			size += getBigBlocksRequired(SMALL_BLOCK_THRESHOLD)
					* BIG_BLOCK_SIZE;
			size += getBigBlocksRequired(SMALL_BLOCK_THRESHOLD)
					* BIG_BLOCK_SIZE;
		...
		System.arraycopy(ps.data, 0, propertySetStorage, pos,
		...
		if (additionalPropertySets != null) {
			ReadPropertyStorage rps = (ReadPropertyStorage) standardPropertySets
					.get(WORKBOOK_NAME);
			previous = rps.propertyStorage.previous != -1 ? mappings[rps.propertyStorage.previous]
					: -1;
			next = rps.propertyStorage.next != -1 ? mappings[rps.propertyStorage.next]
					: -1;
		}
		...
		for (Iterator i = additionalPropertySets.iterator(); i.hasNext();) {
			...
			int block = rps.data.length > SMALL_BLOCK_THRESHOLD ? bigBlock
					: smallBlock;
			...
			next = rps.propertyStorage.next != -1 ? mappings[rps.propertyStorage.next]
					: -1;
			child = rps.propertyStorage.child != -1 ? mappings[rps.propertyStorage.child]
					: -1;
