...
/**
 * A formula record. This is invoked when copying a formula from a read only
 * spreadsheet This method implements the FormulaData interface to allow the
 * copying of writable sheets
 */
class ReadFormulaRecord extends CellValue implements FormulaData {
	...
	/**
	 * An exception has occurred, so produce some appropriate dummy cell
	 * contents. This may be overridden by subclasses if they require specific
	 * handling
	 *
	...
	protected byte[] handleFormulaException() {
		...
		System.arraycopy(expressiondata, 0, data, celldata.length,
	...
	public byte[] getData() {
		...
		try {
			if (parser == null) {
				...
				byte[] formulaBytes = parser.getBytes();
				expressiondata = new byte[formulaBytes.length + 16];
				IntegerHelper.getTwoBytes(formulaBytes.length, expressiondata,
						14);
				System.arraycopy(formulaBytes, 0, expressiondata, 16,
						formulaBytes.length);
			}
			...
			logger.warn(CellReferenceHelper.getCellReference(getColumn(),
					getRow()) + " " + e.getMessage());
	...
	/**
	 * Gets the raw bytes for the formula. This will include the parsed tokens
	 * array. Used when copying spreadsheets
	 *
	 ...
	 *
	 * @param col
	 *            the column which the new cell will occupy
	 * @param row
	...
	/**
	 * Overrides the method in the base class to add this to the Workbook's list
	 * of maintained formulas
	 * 
	 * @param fr
	...
	void columnInserted(Sheet s, int sheetIndex, int col) {
		try {
			if (parser == null) {
				...
				System.arraycopy(formulaData, 16, formulaBytes, 0,
						formulaBytes.length);
				parser = new FormulaParser(formulaBytes, this, getSheet()
						.getWorkbook(), getSheet().getWorkbook(), getSheet()
						.getWorkbookSettings());
				parser.parse();
			}
			...
			logger.warn("cannot insert column within formula:  "
					+ e.getMessage());
		}
	}
	...
	void columnRemoved(Sheet s, int sheetIndex, int col) {
		try {
			...
			logger.warn("cannot remove column within formula:  "
					+ e.getMessage());
		}
	}
	 ...
	 * another sheet are warned appropriately
	 * 
	...
	public boolean handleImportedCellReferences(ExternalSheet es,
			WorkbookMethods mt, WorkbookSettings ws) {
		try {
			if (parser == null) {
				byte[] formulaData = formula.getFormulaData();
				byte[] formulaBytes = new byte[formulaData.length - 16];
				System.arraycopy(formulaData, 16, formulaBytes, 0,
						formulaBytes.length);
				parser = new FormulaParser(formulaBytes, this, es, mt, ws);
				parser.parse();
			}
