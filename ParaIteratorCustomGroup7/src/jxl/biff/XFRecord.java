...
public class XFRecord extends WritableRecordData implements CellFormat {
	...
	/**
	 * The date format for this record. Deduced when the record is read in from
	 * a spreadsheet
	 */
	...
	/**
	 * The format object for this XF record. This is used when creating a
	 * writable record
	 */
	...
	/**
	 * Indicates whether this cell was constructed by an API or read from an
	 * existing Excel file
	 */
	 ...
	 * excel format string back to the user (eg. when generating certain types
	 * of XML document) as opposed to the java equivalent
	 */
	private Format excelFormat;

	/**
	 * Flag to indicate whether the format information has been initialized.
	 * This is false if the xf record has been read in, but true if it has been
	 * written
	 */
	private boolean formatInfoInitialized;

	/**
	 * Flag to indicate whether this cell was copied. If it was copied, then it
	 * can be set to uninitialized, allowing us to change certain format
	 * information
	 */
	private boolean copied;

	/**
	 * A handle to the formatting records. The purpose of this is to read the
	 * formatting information back, for the purposes of output eg. to some form
	 * of XML
	 */
	 ...
	 */
	private static final int[] dateFormats = new int[] { 0xe, 0xf, 0x10, 0x11,
			0x12, 0x13, 0x14, 0x15, 0x16, 0x2d, 0x2e, 0x2f };
	...
	private static final DateFormat[] javaDateFormats = new DateFormat[] {
			...
			new SimpleDateFormat("d-MMM"), new SimpleDateFormat("MMM-yy"),
			new SimpleDateFormat("h:mm a"), new SimpleDateFormat("h:mm:ss a"),
			new SimpleDateFormat("H:mm"), new SimpleDateFormat("H:mm:ss"),
			new SimpleDateFormat("M/d/yy H:mm"), new SimpleDateFormat("mm:ss"),
			new SimpleDateFormat("H:mm:ss"), new SimpleDateFormat("mm:ss.S") };

	/**
	 * The list of built in number format values
	 */
	private static int[] numberFormats = new int[] { 0x1, 0x2, 0x3, 0x4, 0x5,
			0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a,
			0x2b, 0x2c, 0x30 };
	...
	private static NumberFormat[] javaNumberFormats = new NumberFormat[] {
			...
			new DecimalFormat("$#,##0.00;($#,##0.00)"),
			new DecimalFormat("0%"), new DecimalFormat("0.00%"),
			new DecimalFormat("0.00E00"), new DecimalFormat("#,##0;(#,##0)"),
			new DecimalFormat("#,##0;(#,##0)"),
			new DecimalFormat("#,##0.00;(#,##0.00)"),
			new DecimalFormat("#,##0.00;(#,##0.00)"),
			new DecimalFormat("#,##0;(#,##0)"),
	...
	public XFRecord(FontRecord fnt, DisplayFormat form) {
		...
		indentation = 0;
		shrinkToFit = false;
		usedAttributes = (byte) (USE_FONT | USE_FORMAT | USE_BACKGROUND
				| USE_ALIGNMENT | USE_BORDER);
	...
	/**
	 * Copy constructor. Used for copying writable formats, typically when
	 * duplicating formats to handle merged cells
	 *
	 * @param fmt
	 *            XFRecord
	 */
	 ...
	 * different sheets. Unlike the the other copy constructor, this version
	 * does a deep copy
	 *
	 * @param cellFormat
	 *            the format to copy
	...
	/**
	 * Converts the various fields into binary data. If this object has been
	 * read from an Excel file rather than being requested by a user (ie. if the
	 * read flag is TRUE) then no processing takes place and the raw data is
	 * simply returned.
	 *
	...
	/**
	 * Sets the horizontal alignment for the data in this cell. This method
	 * should only be called from its writable subclass CellXFRecord
	 *
	 * @param a
	 *            the alignment
	 */
	...
	/**
	 * Sets the vertical alignment for the data in this cell This method should
	 * only be called from its writable subclass CellXFRecord
	 * 
	 *
	 * @param va
	...
	/**
	 * Sets whether the data in this cell is wrapped This method should only be
	 * called from its writable subclass CellXFRecord
	 *
	 * @param w
	 *            the wrap flag
	 */
	...
	/**
	 * Sets the border for this cell This method should only be called from its
	 * writable subclass CellXFRecord
	 *
	 * @param b
	 *            the border
	 * @param ls
	...
	/**
	 * Gets the line style for the given cell border If a border type of ALL or
	 * NONE is specified, then a line style of NONE is returned
	 *
	 * @param border
	...
	/**
	 * Determines if this cell format has any borders at all. Used to set the
	 * new borders when merging a group of cells
	 *
	...
	/**
	 * If this cell has not been read in from an existing Excel sheet, then
	 * initializes this record with the XF index passed in. Calls initialized on
	 * the font and format record
	 *
	 * @param pos
	...
	/**
	 * Sets the XF index. Called when rationalizing the XF records immediately
	 * prior to writing
	 *
	 * @param xfi
	 *            the new xf index
	...
	private void initializeFormatInformation() {
		...
		if (biffType == biff8) {
			...
			backgroundColour = Colour
					.getInternalColour(colourPaletteMask & 0x3f);
	...
	public int hashCode() {
		...
		hashValue = oddPrimeNumber * hashValue
				+ (bottomBorderColour.getValue());
	...
	/**
	 * Equals method. This is called when comparing writable formats in order to
	 * prevent duplicate formats being added to the workbook
	 *
	 * @param o
	 *            object to compare
	...
	public boolean equals(Object o) {
		...
		if (xfFormatType != xfr.xfFormatType
				|| parentFormat != xfr.parentFormat || locked != xfr.locked
				|| hidden != xfr.hidden || usedAttributes != xfr.usedAttributes) {
			return false;
		}

		if (align != xfr.align || valign != xfr.valign
				|| orientation != xfr.orientation || wrap != xfr.wrap
				|| shrinkToFit != xfr.shrinkToFit
				|| indentation != xfr.indentation) {
			return false;
		}

		if (leftBorder != xfr.leftBorder || rightBorder != xfr.rightBorder
	...
	/**
	 * Sets the font object with a workbook specific clone. Called from the
	 * CellValue object when the font has been identified as a statically shared
	 * font Also called to superimpose a HyperlinkFont on an existing label cell
	 */
