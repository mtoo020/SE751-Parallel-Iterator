...
/**
 * Implementation class for the Range interface. This merely holds the raw range
 * information. This implementation is used for ranges which are present on the
 * current working sheet, so the getSheetIndex merely returns -1
 */
public class SheetRangeImpl implements Range {
	 ...
	 * 
	 * @param s
	 *            the sheet containing the range
	 * @param c1
	 *            the column number of the top left cell of the range
	 * @param r1
	 *            the row number of the top left cell of the range
	 * @param c2
	 *            the column number of the bottom right cell of the range
	 * @param r2
	 *            the row number of the bottomr right cell of the range
	 */
	...
	public Cell getTopLeft() {
		...
		if (column1 >= sheet.getColumns() || row1 >= sheet.getRows()) {
	...
	public Cell getBottomRight() {
		// If the print area exceeds the bounds of the sheet, then handle
		// it here. The sheet implementation will give a NPE
		if (column2 >= sheet.getColumns() || row2 >= sheet.getRows()) {
	...
	/**
	 * Sees whether there are any intersections between this range and the range
	 * passed in. This method is used internally by the WritableSheet to verify
	 * the integrity of merged cells, hyperlinks etc. Ranges are only ever
	 * compared for the same sheet
	 *
	 * @param range
	...
	public boolean intersects(SheetRangeImpl range) {
		...
		if (row2 < range.row1 || row1 > range.row2 || column2 < range.column1
	...
	public boolean equals(Object o) {
		...
		return (column1 == compare.column1 && column2 == compare.column2
				&& row1 == compare.row1 && row2 == compare.row2);
	}
