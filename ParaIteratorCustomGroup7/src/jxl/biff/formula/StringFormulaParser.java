...
class StringFormulaParser implements Parser {
	...
	/**
	 * The stack argument used when parsing a function in order to pass multiple
	 * arguments back to the calling method
	 */
	...
	public StringFormulaParser(String f, ExternalSheet es, WorkbookMethods nt,
			WorkbookSettings ws, ParseContext pc) {
		formula = f;
		settings = ws;
		externalSheet = es;
		nameTable = nt;
	...
	/**
	 * Recursively parses the token array. Recursion is used in order to
	 * evaluate parentheses and function arguments
	 *
	 * @param i
		...
		while (i.hasNext() && !parenthesesClosed) {
			...
			if (pi instanceof Operand) {
				...
				if (operators.empty()) {
					...
					// If the last operator has a higher precedence then add
					// this to
					...
					if (op.getPrecedence() < operator.getPrecedence()) {
						...
						// The operators are of equal precedence, but because it
						// is a
						// unary operator the operand isn't available yet, so
						// put it on
						// the stack
						operators.push(op);
					} else {
						// The operator is of a lower precedence so we can sort
						// out
	...
	private ArrayList getTokens() throws FormulaException {
		...
		try {
			...
			throw new FormulaException(FormulaException.LEXICAL_ERROR, formula
					+ " at char  " + lex.getPos());
		}
	...
	/**
	 * Gets the formula as a string. Uses the parse tree to do this, and does
	 * not simply return whatever string was passed in
	 */
