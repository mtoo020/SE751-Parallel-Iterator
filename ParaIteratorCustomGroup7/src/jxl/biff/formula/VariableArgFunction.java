...
/**
 * A built in function in a formula. These functions take a variable number of
 * arguments, such as a range (eg. SUM etc)
 */
class VariableArgFunction extends Operator implements ParsedThing {
	...
	/**
	 * Flag which indicates whether this was initialized from the client api or
	 * from an excel sheet
	 */
	...
	/**
	 * Adjusts all the relative cell references in this formula by the amount
	 * specified. Used when copying formulas
	 *
	 * @param colAdjust
	...
	byte[] getBytes() {
		...
		System.arraycopy(data, 0, newdata, 0, data.length);
		newdata[data.length] = !useAlternateCode() ? Token.FUNCTIONVARARG
				.getCode() : Token.FUNCTIONVARARG.getCode2();
