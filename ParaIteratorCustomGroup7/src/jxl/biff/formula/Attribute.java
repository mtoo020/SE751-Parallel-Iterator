...
/**
 * A special attribute control token - typically either a SUM function or an IF
 * function
 */
class Attribute extends Operator implements ParsedThing {
	...
	/**
	 * Sets the if conditions for this attribute, if it represents an IF
	 * function
	 *
	 * @param vaf
	 *            a <code>VariableArgFunction</code> value
	 */
	...
	/**
	 * Gets the token representation of this item in RPN. The Attribute token is
	 * a special case, which overrides anything useful we could do in the base
	 * class
	 *
	...
	private byte[] getIf() {
		...
		if (numArgs > 2) {
			...
			IntegerHelper.getTwoBytes(data.length - falseOffsetPos - 2, data,
					falseOffsetPos);
			...
			System.arraycopy(data, 0, newdata, 0, data.length);
			System.arraycopy(falsedata, 0, newdata, data.length,
					falsedata.length);
			data = newdata;
		...
		if (numArgs < 3) {
			// Set the offset to the false expression to be the current position
			IntegerHelper.getTwoBytes(endPos - falseOffsetPos - 5, data,
					falseOffsetPos);
		}
	...
	/**
	 * Gets the precedence for this operator. Operator precedents run from 1 to
	 * 5, one being the highest, 5 being the lowest
	 *
	 ...
	 *
	 * @param colAdjust
	 *            the amount to add on to each relative cell reference
	 * @param rowAdjust
	 *            the amount to add on to each relative row reference
	 */
	...
	/**
	 * Called when a column is inserted on the specified sheet. Tells the
	 * formula parser to update all of its cell references beyond this column
	 *
	 * @param sheetIndex
	 ...
	 * @param currentSheet
	 *            TRUE if this formula is on the sheet in which the column was
	 ...
	 * @param row
	 *            the row number which was inserted1
	 * @param currentSheet
	 *            TRUE if this formula is on the sheet in which the column was
	 *            inserted, FALSE otherwise
	 */
	 ...
	 * another sheet are warned appropriately Does nothing, as operators don't
	 * have cell references
	 */
