...
class TokenFormulaParser implements Parser {
	...
	public TokenFormulaParser(byte[] data, Cell c, ExternalSheet es,
			WorkbookMethods nt, WorkbookSettings ws, ParseContext pc) {
		tokenData = data;
		pos = 0;
		relativeTo = c;
		workbook = es;
	...
	/**
	 * Parses the sublist of tokens. In most cases this will equate to the full
	 * list
	 *
	 ...
	 *
	 * @param len
	 *            the length of the subexpression to parse
	 * @exception FormulaException
	 */
	private void parseSubExpression(int len) throws FormulaException {
		...
		while (pos < endpos) {
			...
			if (t == Token.UNKNOWN) {
				throw new FormulaException(FormulaException.UNRECOGNIZED_TOKEN,
						tokenVal);
			}
			...
			if (t == Token.REF) {
				...
				SharedFormulaCellReference cr = new SharedFormulaCellReference(
						relativeTo);
				pos += cr.read(tokenData, pos);
				tokenStack.push(cr);
			...
			// Functions
				...
				if (vaf.getFunction() != Function.ATTRIBUTE) {
					...
					// This is part of an IF function. Get the operands, but
					// then
	...
	/**
	 * Adds the specified operator to the parse tree, taking operands off the
	...
	/**
	 * Gets the bytes for the formula. This takes into account any token mapping
	 * necessary because of shared formulas
	 *
