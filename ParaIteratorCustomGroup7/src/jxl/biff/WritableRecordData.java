...
/**
 * Extension of the standard RecordData which is used to support those records
 * which, once read, may also be written
 */
public abstract class WritableRecordData extends RecordData implements ByteData {
	 ...
	 *
	 * @param t
	 *            the raw data read from the biff file
	 */
	...
	/**
	 * The number of bytes for this record exceeds the maximum record length, so
	 * a continue is required
	 * 
	 * @param data
	 *            the raw data
	...
	private byte[] handleContinueRecords(byte[] data) {
		...
		for (int i = 0; i < numContinueRecords; i++) {
			...
			int length = Math.min(data.length - oldarraypos,
					maxRecordLength - 4);

			// Add in the continue record code
			IntegerHelper
					.getTwoBytes(Type.CONTINUE.value, newdata, newarraypos);
			IntegerHelper.getTwoBytes(length, newdata, newarraypos + 2);

			// Copy in as much of the new data as possible
			System.arraycopy(data, oldarraypos, newdata, newarraypos + 4,
					length);
	...
	/**
	 * Abstract method called by the getBytes method. Subclasses implement this
	 * method to incorporate their specific binary data - excluding the biff
	 * code and record length, which is handled by this class
	 *
