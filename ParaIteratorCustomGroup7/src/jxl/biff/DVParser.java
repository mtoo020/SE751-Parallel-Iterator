...
public class DVParser {
	 ...
	 * Constructor
	 */
	public DVParser(byte[] data, ExternalSheet es, WorkbookMethods nt,
			WorkbookSettings ws) {
		...
		extendedCellsValidation = (row1 == row2 && column1 == column2) ? false
				: true;
	...
	public DVParser(DVParser copy) {
		...
		if (copy.formula1String != null) {
			...
			try {
				...
				formula2String = (copy.formula2 != null) ? copy.formula2
						.getFormula() : null;
			} catch (FormulaException e) {
				logger.warn("Cannot parse validation formula:  "
						+ e.getMessage());
			}
		}
	...
	String getValidationFormula() throws FormulaException {
		...
		return condition.getConditionString(s1, s2) + "; x "
				+ type.getDescription();
	}
	 ...
	 */
	public void setCell(int col, int row, ExternalSheet es, WorkbookMethods nt,
		...
		formula1 = new FormulaParser(formula1String, es, nt, ws,
				ParseContext.DATA_VALIDATION);
		formula1.parse();

		if (formula2String != null) {
			formula2 = new FormulaParser(formula2String, es, nt, ws,
					ParseContext.DATA_VALIDATION);
			formula2.parse();
		}
	}
	...
	/**
	 * Accessor which indicates whether this validation applies across multiple
	 * cels
	 */
