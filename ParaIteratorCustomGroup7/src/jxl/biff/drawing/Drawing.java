...
public class Drawing implements DrawingGroupObject, Image {
	...
	/**
	 * A reference to the sheet containing this drawing. Used to calculate the
	...
	public static ImageAnchorProperties MOVE_AND_SIZE_WITH_CELLS = new ImageAnchorProperties(
			1);
	public static ImageAnchorProperties MOVE_WITH_CELLS = new ImageAnchorProperties(
			2);
	public static ImageAnchorProperties NO_MOVE_OR_SIZE_WITH_CELLS = new ImageAnchorProperties(
			3);
	 ...
	 *            the object record
	 * @param dd
	 *            the drawing data for all drawings on this sheet
	 * @param dg
	 *            the drawing group
	 */
	public Drawing(MsoDrawingRecord mso, ObjRecord obj, DrawingData dd,
			DrawingGroup dg, Sheet s) {
		drawingGroup = dg;
	...
	private void initialize() {
		...
		if (clientAnchor == null) {
			...
			imageAnchorProperties = ImageAnchorProperties
					.getImageAnchorProperties(clientAnchor.getProperties());
		}
	...
	/**
	 * Accessor for the image file path. Normally this is the absolute path of a
	 * file on the directory system, but if this drawing was constructed using
	 * an byte[] then the blip id is returned
	 *
	...
	/**
	 * Sets the object id. Invoked by the drawing group when the object is added
	 * to id
	 *
	 * @param objid
	 *            the object id
	...
	public EscherContainer getSpContainer() {
		...
		ClientAnchor clientAnchor = new ClientAnchor(x, y, x + width, y
				+ height, imageAnchorProperties.getValue());
	...
	/**
	 * Sets the drawing group for this drawing. Called by the drawing group when
	 * this drawing is added to it
	 *
	 * @param dg
	 *            the drawing group
	 */
	 ...
	 * objects have been written Does nothing here
	 *
	 * @param outputFile
	 *            the output file
	 */
	...
	/**
	 * Accessor for the first drawing on the sheet. This is used when copying
	 * unmodified sheets to indicate that this drawing contains the first time
	 * Escher gubbins
	 *
	 ...
	 * drawings records spread over TXO and CONTINUE records and require special
	 * handling
	 *
	...
	private double getWidthInPoints() {
		...
		double pointSize = (cellView.getFormat() != null) ? cellView
				.getFormat().getFont().getPointSize() : DEFAULT_FONT_SIZE;
		double firstColWidthInPoints = firstColImageWidth * 0.59 * pointSize
				/ 256;
		...
		if (lastCol != firstCol) {
			...
			pointSize = (cellView.getFormat() != null) ? cellView.getFormat()
					.getFont().getPointSize() : DEFAULT_FONT_SIZE;
		...
		// Add on the first and last row contributions to get the height in
		// twips
		double widthInPoints = width + firstColWidthInPoints
				+ lastColWidthInPoints;
	...
	public double getWidth(LengthUnit unit) {
		...
		return widthInPoints
				* LengthConverter.getConversionFactor(LengthUnit.POINTS, unit);
	}
	...
	/**
	 * Gets the horizontal resolution of the image, if that information is
	 * available.
	 *
	...
	/**
	 * Gets the vertical resolution of the image, if that information is
	 * available.
	 *
	 * @return the number of dots per unit specified, if available, 0 otherwise
	 */
