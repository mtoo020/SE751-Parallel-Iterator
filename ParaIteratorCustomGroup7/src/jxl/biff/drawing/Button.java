...
/**
 * Contains the various biff records used to copy a Button (from the Form
 * toolbox) between workbook
 */
public class Button implements DrawingGroupObject {
	 ...
	 *            the workbook settings
	 */
	public Button(MsoDrawingRecord msodr, ObjRecord obj, DrawingData dd,
	...
	public EscherContainer getSpContainer() {
		...
		/*
		 * if (spContainer == null) { spContainer = new SpContainer(); Sp sp =
		 * new Sp(type, shapeId, 2560); spContainer.add(sp); Opt opt = new
		 * Opt(); opt.addProperty(344, false, false, 0); // ?
		 ...
		 * Math.max(0, row - 0.6), column+3, row + 4);
		 * 
	 ...
	 * objects have been written Writes out all the note records
	 *
	 * @param outputFile
	 *            the output file
	 */
	...
	/**
	 * Accessor for the row. As buttons are not associated with a cell, does
	 * nothing here
	 *
	...
	/**
	 * Accessor for the column. As buttons are not associated with a cell, does
	 * nothing here
	 *
	...
	public String getText() {
		if (commentText == null) {
			...
			if (td[0] == 0) {
				commentText = StringHelper.getString(td, td.length - 1, 1,
						workbookSettings);
			} else {
				commentText = StringHelper.getUnicodeString(td,
						(td.length - 1) / 2, 1);
			}
		}
