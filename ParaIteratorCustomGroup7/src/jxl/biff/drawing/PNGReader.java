...
public class PNGReader {
	...
	private static byte[] PNG_MAGIC_NUMBER = new byte[] { (byte) 0x89,
	...
	void read() {
		...
		while (pos < pngData.length) {
			int length = getInt(pngData[pos], pngData[pos + 1],
					pngData[pos + 2], pngData[pos + 3]);
			ChunkType chunkType = ChunkType.getChunkType(pngData[pos + 4],
					pngData[pos + 5], pngData[pos + 6], pngData[pos + 7]);
		...
		if (phys != null) {
			...
			verticalResolution = getInt(physData[4], physData[5], physData[6],
					physData[7]);
		}
	}
