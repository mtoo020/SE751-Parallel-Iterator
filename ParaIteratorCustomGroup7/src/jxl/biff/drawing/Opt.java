...
class Opt extends EscherAtom {
	...
	private void readProperties() {
		...
		for (int i = 0; i < numProperties; i++) {
			...
			Property p = new Property(id, (val & 0x4000) != 0,
					(val & 0x8000) != 0, value);
			pos += 6;
			properties.add(p);
		}
	...
	byte[] getData() {
		...
		for (Iterator i = properties.iterator(); i.hasNext();) {
			...
			if (p.complex && p.stringValue != null) {
				byte[] newData = new byte[data.length + p.stringValue.length()
						* 2];
				System.arraycopy(data, 0, newData, 0, data.length);
				StringHelper.getUnicodeBytes(p.stringValue, newData,
						data.length);
				data = newData;
			}
		}
	 ...
	 *            the blip id
	 * @param complex
	 *            whether it's a complex property
	 * @param val
	 *            the value
	 */
