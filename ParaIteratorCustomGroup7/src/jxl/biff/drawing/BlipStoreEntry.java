...
class BlipStoreEntry extends EscherAtom {
	...
	/**
	 * Flag to indicate that this entry was specified by the API, and not read
	 * in
	 */
	...
	public BlipStoreEntry(EscherRecordData erd) {
		...
		write = false;
		byte[] bytes = getBytes();
		referenceCount = IntegerHelper.getInt(bytes[24], bytes[25], bytes[26],
				bytes[27]);
	}
	...
	byte[] getImageData() {
		...
		System.arraycopy(allData, IMAGE_DATA_OFFSET, imageData, 0,
				imageData.length);
		return imageData;
	}
}
