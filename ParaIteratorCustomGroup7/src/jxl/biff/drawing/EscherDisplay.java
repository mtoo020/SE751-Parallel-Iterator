...
/**
 * Class used to display a complete hierarchically organized Escher stream The
 * whole thing is dumped to System.out
 *
...
public class EscherDisplay {
	...
	private void displayRecord(EscherRecord er, int level) throws IOException {
		...
		if (type == EscherRecordType.DGG_CONTAINER) {
			...
			writer.write("Sp:  shape id " + sp.getShapeId() + " shape type "
					+ sp.getShapeType());
			writer.newLine();
			...
			if (p260 != null) {
				writer.write("260: " + p260.value + ", " + p260.stringValue
						+ ";");
			}
			if (p261 != null) {
				writer.write("261: " + p261.value + ", " + p261.stringValue
						+ ";");
			}
			writer.newLine();
