...
/**
 * Handles the writing out of the different charts and images on a sheet. Called
 * by the SheetWriter object
 */
public class SheetDrawingWriter {
	 ...
	 * @param mod
	 *            flag indicating whether the drawings have been tampered with
	 */
	...
	/**
	 * Writes out the MsoDrawing records and Obj records for each image and
	 * chart on the sheet
	 *
	 * @param outputFile
	 *            the output file
	...
	public void write(File outputFile) throws IOException {
		...
		if (numImages == 0 && charts.length == 1
		...
		for (int i = 0; i < charts.length; i++) {
			...
			byte[] data = spContainer.getBytes(); // use getBytes instead of
													// getData
		...
		int len = IntegerHelper.getInt(firstMsoData[4], firstMsoData[5],
				firstMsoData[6], firstMsoData[7]);
		IntegerHelper.getFourBytes(len + length, firstMsoData, 4);

		// Adjust the length of the SpgrContainer
		len = IntegerHelper.getInt(firstMsoData[28], firstMsoData[29],
				firstMsoData[30], firstMsoData[31]);
		...
		if (numImages > 0) {
			DrawingGroupObject firstDrawing = (DrawingGroupObject) drawings
					.get(0);
	...
	private void writeUnmodified(File outputFile) throws IOException {
		...
		int length = 0;
		EscherContainer[] spContainers = new EscherContainer[numDrawings
				+ charts.length];
