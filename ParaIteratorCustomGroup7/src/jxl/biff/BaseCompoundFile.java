...
public abstract class BaseCompoundFile {
   ...
   */
    ...
    */
	...
	public final static String[] STANDARD_PROPERTY_SETS = new String[] {
			ROOT_ENTRY_NAME, WORKBOOK_NAME, SUMMARY_INFORMATION_NAME,
	...
	/**
	 * Inner class to represent the property storage sets. Access is public to
	 * allow access from the PropertySetsReader demo utility
	 */
	public class PropertyStorage {
		...
		public PropertyStorage(byte[] d) {
			...
			startBlock = IntegerHelper.getInt(data[START_BLOCK_POS],
					data[START_BLOCK_POS + 1], data[START_BLOCK_POS + 2],
					data[START_BLOCK_POS + 3]);
			size = IntegerHelper.getInt(data[SIZE_POS], data[SIZE_POS + 1],
					data[SIZE_POS + 2], data[SIZE_POS + 3]);
			previous = IntegerHelper.getInt(data[PREVIOUS_POS],
					data[PREVIOUS_POS + 1], data[PREVIOUS_POS + 2],
					data[PREVIOUS_POS + 3]);
			next = IntegerHelper.getInt(data[NEXT_POS], data[NEXT_POS + 1],
					data[NEXT_POS + 2], data[NEXT_POS + 3]);
			child = IntegerHelper.getInt(data[CHILD_POS], data[CHILD_POS + 1],
					data[CHILD_POS + 2], data[CHILD_POS + 3]);
		...
		public PropertyStorage(String name) {
			...
			IntegerHelper.getTwoBytes((name.length() + 1) * 2, data,
					SIZE_OF_NAME_POS);
