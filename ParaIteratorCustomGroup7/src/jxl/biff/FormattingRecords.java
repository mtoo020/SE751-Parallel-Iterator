...
public class FormattingRecords {
	...
	/**
	 * A hash map of FormatRecords, for random access retrieval when reading in
	 * a spreadsheet
	 */
	...
	/**
	 * The maximum number of format records. This is some weird internal Excel
	 * constraint
	 */
	 ...
	 * xfRecords list, and this position is passed to the XF records initialize
	 * method
	 *
	 * @param xf
	 *            the xf record to add
	 ...
	 * represents a date. First checks against the built in formats, and then
	 * checks against the hash map of FormatRecords
	 *
	 * @param pos
	 *            the xf format index
	...
	public final boolean isDate(int pos) {
		...
		FormatRecord fr = (FormatRecord) formats.get(new Integer(xfr
				.getFormatRecord()));
	 ...
	 *
	 * @param index
	 *            the formatting record index to retrieve
	 * @return the format record at the specified index
	 */
	FormatRecord getFormatRecord(int index) {
		return (FormatRecord) formats.get(new Integer(index));
	}
	 ...
	 * writable subclass because there is an upper limit on the amount of format
	 * records that are allowed to be present in an excel sheet
	 *
	...
	/**
	 * Rationalizes the cell formats. Duplicate formats are removed and the
	 * format indexed of the cells adjusted accordingly
	 *
	 * @param fontMapping
	 *            the font mapping index numbers
	 * @param formatMapping
	 *            the format mapping index numbers
	 * @return the list of new font index number
	 */
	public IndexMapping rationalize(IndexMapping fontMapping,
		...
		for (Iterator it = xfRecords.iterator(); it.hasNext();) {
			...
			if (xfr.getFormatRecord() >= customFormatStartIndex) {
				xfr.setFormatIndex(formatMapping.getNewIndex(xfr
						.getFormatRecord()));
			}
		...
		for (int i = minXFRecords; i < xfRecords.size(); i++) {
			...
			for (Iterator it = newrecords.iterator(); it.hasNext()
					&& !duplicate;) {
	...
	/**
	 * Rationalizes the display formats. Duplicate formats are removed and the
	 * format indices of the cells adjusted accordingly. It is invoked
	 * immediately prior to writing writing out the sheet
	 * 
	...
	public IndexMapping rationalizeDisplayFormats() {
		...
		while (i.hasNext()) {
			...
			if (!duplicate) {
				...
				mapping.setMapping(df.getFormatIndex(), df.getFormatIndex()
						- numremoved);
			}
		}
