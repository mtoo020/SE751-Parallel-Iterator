...
/**
 * Class which encapsulates a data validation. This encapsulates the base DVAL
 * record (DataValidityListRecord) and all the individual DV
...
public class DataValidation {
	 ...
	 */
	public DataValidation(int objId, ExternalSheet es, WorkbookMethods wm,
			WorkbookSettings ws) {
		workbook = wm;
		externalSheet = es;
		workbookSettings = ws;
	 ...
	 */
	public DataValidation(DataValidation dv, ExternalSheet es,
		...
		for (int i = 0; i < settings.length; i++) {
			validitySettings.add(new DataValiditySettingsRecord(settings[i],
					externalSheet, workbook, workbookSettings));
		}
	}
	...
	public void write(File outputFile) throws IOException {
		if (validitySettings.size() > MAX_NO_OF_VALIDITY_SETTINGS) {
			logger.warn("Maximum number of data validations exceeded - "
					+ "truncating...");
			validitySettings = new ArrayList(validitySettings.subList(0,
					MAX_NO_OF_VALIDITY_SETTINGS - 1));
		...
		for (Iterator i = validitySettings.iterator(); i.hasNext();) {
			DataValiditySettingsRecord dvsr = (DataValiditySettingsRecord) i
					.next();
	...
	public void insertRow(int row) {
		for (Iterator i = validitySettings.iterator(); i.hasNext();) {
			DataValiditySettingsRecord dv = (DataValiditySettingsRecord) i
					.next();
			dv.insertRow(row);
		}
	}
	 ...
	 *            the first row
	 */
	public void removeSharedDataValidation(int col1, int row1, int col2,
			int row2) {
		for (Iterator i = validitySettings.iterator(); i.hasNext();) {
			DataValiditySettingsRecord dv = (DataValiditySettingsRecord) i
					.next();
	...
	/**
	 * Used during the copy process to retrieve the validity settings for a
	 * particular cell
	 */
